...
net_utils.prototype.addToNetwork = function(netuid, board, value, res){
    ...
    logger.info("ADDING board "+ board + " to network " + " ...");
	  ...
	  logger.info("NETWORK COMMAND: ip link set br"+network.uuid.substring(0, 6)+" down");
		...
		function(resTunnel){
		  ...
		  if (resTunnel==="OK-TUNNEL"){
			...
			response.result = "TUNNEL SUCCESSFULLY ESTABLISHED!";
			res.send(JSON.stringify(response,null,"\t"));
			
		  } else{
		    logger.info("PROBLEMS DURING TUNNEL CREATION!");
		    response.result = "PROBLEMS DURING TUNNEL CREATION!";
		  }
	      ...
	      response.result = "board already configured!";
...
net_utils.prototype.showBoards = function(netuid, res){
	...
	logger.info("SHOW BOARDS IN THE NETWORK: "+netuid);
...
function createEstablishTunnels(session,topic_command,elem, network){
  ...
  //return function() {
    ...
    logger.info('NETWORK COMMAND: socat -d -d TCP:localhost:'+elem.socatPort+',reuseaddr,forever,interval=10 TUN:'+elem.socatServ+'/30,up');
    ...
    servSocat.stderr.on('data', function (data) {
      ...
      var textdata = 'SOCAT - stderr: ' + data;
      ...
      if(textdata.indexOf("starting data transfer loop") > -1){
		  ...
		  logger.info('NETWORK COMMAND: ip link set '+elem.greTap+' up');
                  ...
                  testing2.on('close', function (code) {
                    ...
                    logger.info('--> turn if up child process exited with code ' + code);
		    ...
		    logger.info('NETWORK COMMAND: brctl addif br'+elem.greTap.substring(0, 6) + " " + elem.greTap);
                    ...
                    testing3.on('close', function (code) {
                      ...
                      logger.info('--> turn up child process exited with code ' + code);
    ...
    servSocat.on('close', function (code) {
      ...
      logger.info('ip link del '+elem.greTap);
      ...
      logger.info('SOCAT child process exited with code ' + code);
    });
