...
net_utils = function (session, rest) {
    ...
    rest.put('/v1/vnets/:board/force', function (req, res) {
        ...
        logger.info("[NETWORK] - Activating VNETs on board " + board + "...");
    ...
    db.getBoardVLAN(board, function (data) {
        ...
        if (data.result == "ERROR") {
            ...
            if (data.message.length != 0) {
                ...
                logger.debug("[NETWORK] --> " + JSON.stringify(data.message));
                res.send(JSON.stringify(data.message));

            }else {
                response.result = "SUCCESS";
                response.message = data.message;
                logger.debug("[NETWORK] --> The "+board+" is not in any VNET!");
                    ...
                    db.getVnetBoardsInfo(net_uuid, function (boardsList) {
                        ...
                        if (boardsList.result == "ERROR") {
                            ...
                            if (boardsNumber != 0) {
                                ...
                                logger.debug("[NETWORK] --> No boards to remove from this VNET!");
                                destroyVLAN(net_uuid, res, response);
...
net_utils.prototype.getVnetBoardsInfo = function (net_uuid, res) {
    ...
    logger.info("[NETWORK] - Showing boards in the VNET: " + net_uuid);
                ...
                if (network.message.length == 0) {
                    ...
                    db.getVnetBoardsInfo(net_uuid, function (db_response) {
                        ...
                        if (db_response.result == "ERROR") {
                            ...
                            if (db_response.message.length != 0) {
                                logger.info("[NETWORK] --> VNET Boards info: " + JSON.stringify(db_response.message, null, "\t"));
                                res.send(JSON.stringify(db_response, null, "\t"));

                            } else {
                                response.result = "SUCCESS";
                                response.message = db_response.message; //"No boards in this VNET!";
                                logger.info("[NETWORK] --> No boards in this VNET!");
                                res.send(JSON.stringify(response));
            ...
            db.getNetEnabledId(board, function (data) {
                ...
                if (data.result == "ERROR") {
                    ...
                    if (net_enabledID == 1) {
                        ...
                        if (value == "") {
                            logger.info("[NETWORK] - Adding board " + board + " to VNET " + netuuid + "...");
                        } else {
                            logger.info("[NETWORK] - Adding board " + board + " to VNET " + netuuid + " with " + value + " ...");
                        }

                        logger.debug("[NETWORK] - Board " + board + " is network enabled!");
                        ...
                        db.checkAddressPool(netuuid, function (free_address) {
                            ...
                            if(free_address.result == "ERROR"){
                                ...
                                //NO IP available
                                logger.warn("[NETWORK] --> Adding board: No IP available for this VNET " + netuuid + " !!!");
                                            ...
                                            if (netsize >= 1) {
                                                ...
                                                db.checkBoardIntoVLAN(board, netuuid, function (response_db) {
                                                    ...
                                                    if (response_db.result == "ERROR") {
                                                        ...
                                                        logger.debug("[NETWORK] ----> Board in VNET: " + response_db.message[0].found);
                                                                        ...
                                                                        db.getFreeAddress(netuuid, function (response_db) {
                                                                            ...
                                                                            if (response_db.result == "ERROR") {
                                                                                ...
                                                                                if (response_db.message.length == 0) {
                                                                                    ...
                                                                                    logger.error("[NETWORK] --> Adding board: No IP available for this VNET " + network.message[0].net_uuid + " !!!");
                                                                                    response.result = "ERROR";
    ...
    session_wamp.call(elem.board + '.command.rpc.network.addToNetwork', [elem.board, elem.vlanID, elem.greIP, elem.greMask, elem.vlan_name]).then(
        ...
        function (result) {
            ...
            tag_bridge_iface.on('close', function (code) {
                ...
                logger.info('[NETWORK] --> Board ' + elem.board + ' added to VNET ' + elem.vlan_name + ' with IP ' + elem.greIP);
    ...
    if (net_uuid == undefined) {
        ...
        db.BoardInfo(board, function (response_db) {
            ...
            if (response_db.result == "ERROR") {
                ...
                logger.debug("[NETWORK] --> Board label: " + JSON.stringify(board_label));
                ...
                db.checkBoardIntoVLAN(board, net_uuid, function (response_db) {
                    ...
                    if (response_db.result == "ERROR") {
                        ...
                        logger.debug("[NETWORK] --> Board "+board_label+" in VNET: " + response_db.message[0].found);
                        ...
                        if (response_db.message[0].found == 1) {
                            ...
                            response.result = "WARNING";
                            logger.warn("[NETWORK] --> Board " + board_label + " is not connected to the specified network: "+net_uuid);
                            res.send(JSON.stringify(response));
                        }
    ...
    db.getNetEnabledId(db_board_id, function (data) {
        ...
        if (data.result == "ERROR") {
            ...
            if (net_enabledID == 1) {
                ...
                logger.info("[NETWORK] - Board " + db_board_id + " is network enabled!");
                    ...
                    if (data.result == "ERROR") {
                        ...
                        if (data.message[0] == undefined || data.message[0].status == "noactive" || restore == "true" ) {
                            ...
                            if (data.message[0] == undefined) {
                                ...
                                logger.debug("[NETWORK] --> Board " + db_board_id + " Socat status: " + data.message[0].status);
                            }
                            ...
                            db.getSocatConf(db_board_id, basePort, socatNetwork, function (data) {
                                ...
                                if (data.result == "ERROR") {
                                    ...
                                    logger.info("[NETWORK] --> Injecting Socat parameters into board " + db_board_id + ":\n"+JSON.stringify(data.message, null, "\t"));
                                    ...
                                    session_wamp.call(db_board_id + '.command.rpc.network.setSocatOnBoard', [socatServAddr, socatPort, socatBoardAddr, net_backend]).then(
                                        ...
                                        function (result) {
                                            ...
                                            logger.info("[NETWORK] --> Socat result from board:\n" + JSON.stringify(result, null, "\t"));
                                                            ...
                                                            db.updateSocatStatus(db_board_id, "active", function (data) {
                                                                if (data.result == "ERROR") {
                                                                    ...
                                                                    if (restore == "true") {
                                                                        ...
                                                                        response.result = "SUCCESS";
                                                                        res.send(JSON.stringify(response));
                                                                        logger.info("[NETWORK] - Board " + db_board_id + " network restored!");
                            ...
                            logger.info("[NETWORK] - BOARD " + db_board_id + " ALREADY CONNECTED TO SOCAT - status: " + data.message[0].status);
                ...
                response.message = "BOARD " + db_board_id + " NO NETWORK ENABLED!";
                response.result = "ERROR";
                logger.debug("[NETWORK] - " + response.message);
                res.send(JSON.stringify(response));
