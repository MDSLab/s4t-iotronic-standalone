...
net_utils.prototype.showBoards = function(net_uuid, res){
    ...
    //select id_board, vlans_connection.ip_vlan, vlans_connection.id_vlan from socat_connections,vlans_connection, vlans where socat_connections.id = vlans_connection.id_socat_connection AND id_vlan = vlans.id AND vlans.net_uuid="f159f0fd-00e7-4b3c-a4c3-224cd17fdb91";
    ...
    if(net_uuid == undefined) {
      ...
      var position = findValue(networksArray, net_uuid, 'uuid');
	...
	logger.info("SHOW BOARDS IN THE NETWORK: "+net_uuid + "- network.nethosts: " + JSON.stringify(network.nethosts));
