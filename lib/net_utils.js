...
net_utils = function (session, rest) {
    ...
    //get node vnets list
    rest.get('/v1/nodes/:node/vnets', function (req, res) {

        logger.debug("[API] - Node VNETs list - " + Object.keys( req.route.methods ) + " - " + req.route.path);

        var node = req.params.node;
        net_utils.prototype.showNodeNetwork(node, res);

    });

    //show vnet nodes list
    ...
    //add node to vnet
    rest.put('/v1/nodes/:node/vnets/:vnet', function (req, res) {
    ...
    rest.delete('/v1/nodes/:node/vnets/:vnet', function (req, res) {
...
net_utils.prototype.showNodeNetwork = function (node, res) {
    ...
    db.getNodeVLAN(node, function (data) {
        ...
        if (data.result == "ERROR") {
            ...
            if (data.message.length != 0) {
                ...
                logger.debug("[NETWORK] --> The "+node+" is not in any VNET!");
                ...
                db.getSocatStatus(db_board_id, function (data) {
                    ...
                    if (data.result == "ERROR") {
                        ...
                        if (data.message[0] == undefined || data.message[0].status == "noactive" || restore == "true") {
                            ...
                            logger.info("[NETWORK] - NODE " + db_board_id + " ALREADY CONNECTED TO SOCAT - status: " + data.message[0].status);
