	...
	db.getNetEnabledId(db_board_id, function(data){
		...
		if( data[0] == undefined || data[0].status == "noactive" || restore == "true"){
			...
			db.getSocatConf(db_board_id, basePort, socatNetwork, function(data){
				...
				//NEW-net
				session_wamp.call(db_board_id+'.command.rpc.network.setSocatOnBoard', [ socatServAddr, socatPort, socatBoardAddr]).then(
					...
					greIface.on('close', function (code) {
						...
						add_to_brgre.on('close', function (code) {
							...
							logger.info("TUNNELS CONFIGURATION SERVER SIDE FOR BOARD "+db_board_id+" COMPLETED!"); 
							...
							if(restore == "true"){
							    ...
							    logger.info("Board "+board+" network restored!");
...
//net_utils.prototype.addToNetwork = function(netuid, board, value, res){
	...
	db.getNetEnabledId(board, function(data){
	    ...
	    if(net_enabledID == 1){
	      ...
	      logger.info("BOARD "+board+" NETWORK ENABLED!");
		...
		db.checkAddressPool(netuid, function(free_address){
		  ...
		  logger.info("free_address SELECTED: "+ JSON.stringify(free_address));
		  
		  if (free_address.result=="NO-IP" && restore == "false" ){
		    //NO IP available
		  
		    logger.warn("--> ADDING board: NO IP AVAILABLE for this network " + netuid + " !!!");
			  ...
			  db.getVlan(netuid, function(network){
			    ...
			    if(network.length != 0){
			      ...
			      if(netsize >= 1){
					  ...
					  logger.info("checkBoardIntoVLAN response: "+ JSON.stringify(response_db));
					  logger.info("--> checkBoardIntoVLAN: " + response_db.message[0].found);
					  ...
					  if(response_db.message[0].found == 0 || restore == "true") { //if the board isn't configured
					      ...
					      db.getSocatConn(board, function(soc_result){
							...
							if (restore == "false"){
							      ...
							      logger.info("IP specified: "+ value +" for the network "+network[0].id);
							      ...
							      db.checkAssignedVlanIP(value, network[0].id, function(response_db){
								    ...
								    logger.info("checkAssignedVlanIP response: "+ JSON.stringify(response_db));
								    ...
								    if(response_db.message[0].found == 1){
								      ...
								      logger.warn("IP "+ value +" is NOT AVAILABLE!");
							      ...
							      logger.info("IP "+ value +" restoring...");
							      createEstablishTunnels(elem, res, response, restore);
							...
							db.getFreeAddress(netuid, function(response_db){
								  ...
								  logger.warn("--> ADDING board: NO IP AVAILABLE for this network " + network[0].net_uuid + " !!!");
								  response.result = "WARNING";
								  response.log = "NO IP AVAILABLE for this network!";
								  res.send(JSON.stringify(response,null,"\t"));
					    ...
					    logger.warn("--> ADDING board: board already configured!");
					    response.result = "WARNING";
	      ...
	      logger.info("BOARD "+board+" NO NETWORK ENABLED!");
