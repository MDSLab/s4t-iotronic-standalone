...
//Function to get the name and code of a generic driver
db_utils.prototype.getBoardPositionHistory = function (board_id, callback) {
    conn();
    var response = {
        message: '',
        result: ''
    };
    connection.query("SELECT altitude, longitude, latitude, timestamp FROM coordinates WHERE coordinates.board_id = '" + board_id + "' AND timestamp BETWEEN CURRENT_DATE - INTERVAL 1 DAY ", function (err, result) {
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            disconn();
