...
db_utils.prototype.BoardInfo = function (board, callback) {
    ...
    connection.query("SELECT boards.board_code, boards.label, boards.latest_update, boards.description, boards.mobile, users.username as user, projects.name as project, layouts.model, layouts.manufacturer, layouts.image, boards.net_enabled, coordinates.altitude, coordinates.longitude, coordinates.latitude " +
...
db_utils.prototype.getBoardPosition = function (board_id, callback) {
    ...
    connection.query("SELECT altitude, longitude, latitude FROM coordinates WHERE coordinates.board_id = '" + board_id + "' ORDER BY timestamp DESC LIMIT 1", function (err, result) {
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            disconn();
...
db_utils.prototype.getUserByBoard = function (board_id, callback) {
    ...
    connection.query("SELECT * FROM users, boards WHERE boards.users_id = users.uuid", function (err, result) {
