...
db_utils.prototype.insertCreatedPlugin = function (pluginname, plugincategory, pluginjsonschema, plugincode, callback) {
    ...
    connection.query("INSERT INTO plugins (name, category, jsonschema, code) VALUES (" + mysql.escape(pluginname) + " , " + mysql.escape(plugincategory) + " , " + mysql.escape(pluginjsonschema) + " , " + mysql.escape(plugincode) + ")", function (err, result) {
        ...
        if (err != null) {
            response.message = err;
            response.result = "ERROR";
            //logger.error("[PLUGIN] --> "+ response.result + " - " + err);
            callback(response);
        } else {
            response.message = result;
            response.result = "SUCCESS";
...
db_utils.prototype.insertCreatedMeasure = function (measurename, readpluginId, elaboratepluginId, callback) {
    ...
    connection.query("INSERT INTO measures (name, read_plugin, elaborate_plugin) VALUES ('" + measurename + "','" + readpluginId + "','" + elaboratepluginId + "')", function (err, result) {
        //console.log(err);
        if (err != null) {
            console.log(err);
        }
