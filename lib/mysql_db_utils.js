...
db_utils.prototype.regBoard = function(board, latitude, longitude, altitude, net_enabled, sensorList, callback){
...
//Function to register a new board
db_utils.prototype.updateBoard = function(board, latitude, longitude, altitude, net_enabled, sensorList, callback){
	...
	logger.info("--> Board ID check...");
	    ...
	    if(result.length != 0){
	      ...
	      connection.query("UPDATE boards_connected, user_boards SET user_boards.net_enabled='"+net_enabled+"' ,boards_connected.altitude='"+altitude+"', boards_connected.longitude='"+longitude+"', boards_connected.latitude='"+latitude+"' WHERE user_boards.board_code='"+board+"' AND boards_connected.board_code='"+board+"'", function(err, latest_result){
				  ...
				  for(var i = 0; i < sensorList2.length; i++) {
					...
					connection.query("INSERT INTO sensors_on_board (id_sensor, id_board) VALUES ('"+sensorList2[i]+"', '"+board+"')", function(err, latest_result){
					  ...
					  if( i == (sensorList2.length - 1) ){
					    ...
					    callback("Updating board successfully completed!");
...
db_utils.prototype.getBoardInfo = function(board, callback){
	...
	connection.query("SELECT altitude, longitude, latitude, net_enabled FROM boards_connected, user_boards WHERE boards_connected.board_code = '" + board + "' AND boards_connected.board_code = user_boards.board_code", function(err_info, result_info){
...
db_utils.prototype.insertVLANConnection = function(vlan_ip , net_uuid, id_board, callback){
	...
	conn();
		...
		if(err != null){
		      logger.error("insertNetVlan - SELECT error in checking ip_vlans: "+ err);
		      disconn();
                      response.message=err;
                      response.result='ERROR';
                      callback(respone);
                    ...
                    if (result[0].found == 1) {
                            logger.info("--> IP already exists: "+ result[0].id);
                            disconn();
                            response.message=result;
			    ...
			    callback(respone);
                            ...
                            logger.info("--> registering new VLAN Connection in DB...");	
                            ...
                            connection.query("INSERT INTO vlans_connection (id, id_vlan, id_socat_connection,ip_vlan) VALUES ('', (select id from vlans where net_uuid='"+net_uuid+"'), (select id from socat_connections where id_board="+id_board+"), '"+vlan_ip+"'))", function(err,result){
                                    ...
                                    if(err != null){
                                        ...
                                        logger.error("insertNetVlan - INSERT error in vlans_connection tables: "+ err);
                                        disconn();
                                        response.message=err;
                                        response.result='ERROR';
                                        callback(respone);
...
db_utils.prototype.insertAddresses = function(list_ip, net_uuid, callback){
	...
	logger.info("--> registering new VLAN Addresses in DB...");
        ...
        var query="INSERT INTO free_addresses (vlans_id, ip) VALUES "
	...
	connection.query(getVID, function(err, result){
		...
		if(err != null){
			  ...
			  (function(i) {
			      ...
			      v = util.format("(%d,'%s')", result[0].id, list_ip[i]);
			      query=query+v;
...
db_utils.prototype.checkBoardIntoVLAN = function(board, net_uuid, callback){
	...
	connection.query("SELECT EXISTS(  SELECT id_vlan, id_socat_connection FROM vlans_connection WHERE id_vlan = (SELECT id FROM vlans WHERE net_uuid='" + net_uuid + "' )  AND (SELECT id FROM socat_connections WHERE id_board='" + board + "' )           ) AS found", function(err, result){
...
db_utils.prototype.getFreeAddress = function(net_uuid, callback){
	...
	var response = {
	  ...
	  result: ''
        ...
        logger.info("net_uuid - " + net_uuid);
	query="select ip from free_addresses where vlans_id = (SELECT id FROM vlans WHERE net_uuid='" + net_uuid + "' ) limit 1";
...
db_utils.prototype.checkAssignedVlanIP = function(user_ip, vlan_id, callback){
	...
	connection.query("SELECT EXISTS(  SELECT vlans_id, ip FROM free_addresses WHERE vlans_id ='" + vlan_id + "', ip ='" + user_ip + "' ) AS found", function(err, result){
