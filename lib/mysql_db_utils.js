...
db_utils.prototype.insertPort = function(port, node_uuid, callback){
	...
	logger.info("[NETWORK] --> Saving the port into the db " + port.id);
        ...
        var query=util.format("INSERT INTO ports (id,uuid,mac_address,ip_address,node_uuid,net_uuid) VALUES ('','%s','%s','%s','%s','%s')",port.id,port.macAddress,port.fixedIps[0].ip_address,node_uuid,port.networkId);
