        ...
        if (result.length == 0) {
            ...
            connection.beginTransaction(function(err) {
                ...
                if (err) {
                    ...
                    response.result = "ERROR";
                    response.message = err;
                    logger.error("[NETWORK] --> removeBoardFromVlan transaction error: " + err);
                    callback(response);
...
db_utils.prototype.insertNode = function (node_id, session, state, callback) {
    ...
    connection.query("INSERT INTO boards_connected (board_code, session_id, status) VALUES ('" + node_id + "','" + session + "','" + state + "')", function (err, result) {
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            logger.error("insertNode: " + response.message);
            disconn();
            callback(response);
...
db_utils.prototype.insertVNET = function (vlan_ip, vlan_mask, net_uuid, vlan_name, callback) {
    ...
    connection.query("INSERT INTO vlans (vlan_ip, vlan_mask, net_uuid, vlan_name) VALUES ('" + vlan_ip + "', '" + vlan_mask + "', '" + net_uuid + "', '" + vlan_name + "' )", function (err, result) {
        ...
        if (err != null) {
            ...
            response.result = "SUCCESS";
            logger.debug("[NETWORK] ----> insertVNET - new vlan ID ("+result.insertId+") selected from DB.");
            disconn();
            callback(response);
