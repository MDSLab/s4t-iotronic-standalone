...
db_utils.prototype.insertBoard = function (board_id, session, state, callback) {
    conn();
    var response = {
        message: '',
        result: ''
    };
    connection.query("INSERT INTO boards_connected (board_code, session_id, status) VALUES ('" + board_id + "','" + session + "','" + state + "')", function (err, result) {
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            logger.error("insertBoard: " + response.message);
            disconn();
            callback(response);
...
//Function to get the services started in a board
db_utils.prototype.BoardServices = function (board, callback) {
    conn();
    var response = {
        message: '',
        result: ''
    };
    connection.query("SELECT * FROM reverse_cloud_services WHERE reverse_cloud_services.board_id ='" + board + "';", function (err, result) {
        if (err != null) {
            logger.error("BoardServices: " + err);
            response.message = err;
            response.result = "ERROR";
            disconn();
            callback(response);
    ...
    connection.query("SELECT id FROM socat_connections WHERE id_board='" + board + "'", function (err, result) {
        ...
        if (err != null) {
            ...
            if (result[0] == undefined) {
                ...
                logger.debug("[NETWORK] --> registering new board in DB...");
                    ...
                    if (err) {
                        ...
                        connection.query("INSERT INTO socat_connections (id_board, status, port, ip_board, ip_server) VALUES ('" + board + "', 'noactive', 0, 'None', 'None' )", function (err, result) {
                            ...
                            if (err != null) {
                                ...
                                logger.debug("[NETWORK] --> selecting new Socat board ID from DB: bSocatNum = " + bSocatNum);
                                ...
                                logger.debug("[NETWORK] --> creating new NET parameters in DB for board " + board + "..." + " port = '" + socatPort + "', ip_board='" + socatBoardAddr + "', ip_server='" + socatServAddr);
                ...
                connection.query("SELECT id, ip_board, ip_server, port FROM socat_connections WHERE id_board='" + board + "'", function (err, result) {
                    ...
                    if (err != null) {
                        ...
                        //logger.debug("[NETWORK] --> Selecting Socat board ID from DB: bSocatNum = " + bSocatNum);
...
db_utils.prototype.getPort = function (net_uuid, board_uuid, callback) {
    conn();
    logger.info("[NETWORK] --> Getting the port for the board " + board_uuid + " on the network " + net_uuid);
...
db_utils.prototype.getAllPorts = function (board_uuid, callback) {
    conn();
    logger.info("[NETWORK] --> Getting all the ports for the board " + board_uuid);
