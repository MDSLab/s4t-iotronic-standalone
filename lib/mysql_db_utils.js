...
db_utils.prototype.regNode = function (board, board_label, latitude, longitude, altitude, net_enabled, sensorsList, callback) {
    ...
    logger.debug("[SYSTEM] --> SENSOR LIST: " + sensorsList);
    ...
    connection.query("SELECT code FROM board_codes WHERE code =  '" + board + "'", function (err, result) {
        ...
        if (result.length == 0) {
            ...
            connection.query("INSERT INTO board_codes VALUES ('" + board + "', NOW() )", function (err, result2) {
                ...
                if (err != null) {
                    disconn();
                    logger.error("[SYSTEM] --> Registration error in board_codes: " + err);
                    response.result = "ERROR";
                    ...
                    connection.query("INSERT INTO user_boards (user_id, board_type, board_code, cloud_enabled, net_enabled) VALUES (1,1,'" + board + "',1,'" + net_enabled + "');", function (err, result3) {
                        if (err != null) {
                            disconn();
                            logger.error("[SYSTEM] --> Registration error in user_boards: " + err);
                            response.result = "ERROR";
                            ...
                            connection.query("INSERT INTO boards_connected (board_code, label, session_id, status, altitude, longitude, latitude) VALUES ('" + board + "', '" + board_label + "','null','D', '" + altitude + "', '" + longitude + "', '" + latitude + "')", function (err, latest_result) {
                                ...
                                if (err != null) {
                                    disconn();
                                    logger.error("[SYSTEM] --> Registration error in boards_connected: " + err);
                                    response.result = "ERROR";
                                    ...
                                    if (sensorsList.length > 0) {
                                        ...
                                        for (var i = 0; i < sensorsList2.length; i++) {
                                            ...
                                            (function (i) {
                                                ...
                                                logger.debug("[SYSTEM] --> INSERT INTO sensors_on_board (id_sensor, id_board) VALUES ('" + sensorsList2[i] + "', '" + board + "')");


                                                connection.query("INSERT INTO sensors_on_board (id_sensor, id_board) VALUES ('" + sensorsList2[i] + "', '" + board + "')", function (err, latest_result) {

                                                    if (err != null) {
                                                        logger.error("[SYSTEM] --> Registration error in sensors_on_board: " + err);
                                                        response.result = "ERROR";
                                        ...
                                        disconn();
                                        logger.info("[SYSTEM] --> Registration of the board " + board + " successfully completed!");
                                        response.result = "SUCCESS";
                                        response.message = "Registration successfully completed!";
                                        callback(response);
                                    }
            ...
            disconn();
            logger.warn("[SYSTEM] --> The board " + board + " already exists!");
            response.result = "WARNING";
            response.message = "The board " + board + " already exists!";
            callback(response);
        }
                        ...
                        if (err != null) {
                            ...
                            for (var i = 0; i < sensorList2.length; i++) {
                                ...
                                (function (i) {
                                    ...
                                    logger.debug("[SYSTEM] --> INSERT INTO sensors_on_board (id_sensor, id_board) VALUES ('" + sensorList2[i] + "', '" + board + "')");
...
db_utils.prototype.checkNodeConnected = function (node_id, callback) {
    ...
    connection.query("SELECT * FROM boards_connected WHERE boards_connected.board_code ='" + node_id + "'", function (err, result) {
        if (err != null) {
            logger.error("checkNodeConnected "+err);
        }
        disconn();
        callback(result);
    });
    ...
    connection.query("SELECT EXISTS(SELECT 1 FROM vlans_connection WHERE ip_vlan ='" + vlan_ip + "' LIMIT 1) AS found", function (err, result) {
        if (err != null) {
            ...
            if (result[0].found == 1) {
                ...
                logger.debug("[NETWORK] ----> insertVnetConnection - registering new VNET connection...");
...
db_utils.prototype.updateDriverStatus = function (board, drivername, status, callback) {
    ...
    connection.query("SELECT id FROM drivers WHERE name = '" + drivername + "'", function (err, result) {
        ...
        if (err) {
            ...
            callback("driver " + drivername + "updating failed: " + err);
            ...
            connection.query("UPDATE drivers_injected SET state = '" + status + "', latest_change=NOW() WHERE board_id='" + board + "' AND driver_id='" + driver_id + "'", function (err, result) {
                if (err) {
                    ...
                    callback("driver " + drivername + " status: " + status);
                }
                disconn();
            });
...
db_utils.prototype.deleteInjectedDriver = function (board, driver_id, callback) {
    ...
    connection.query("DELETE FROM drivers_injected WHERE board_id='" + board + "' AND driver_id='" + driver_id + "'", function (err, result) {
        if (err) {
            logger.error("deleteInjectedDriver: " + err);
        } else {
            callback("driver_id: " + driver_id);
        }
        disconn();
...
//Function to obtain the driver list
