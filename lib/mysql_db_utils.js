...
//Function to obtain nodes list
db_utils.prototype.getNodesList = function (callback) {
...
//Function to obtain node info
db_utils.prototype.NodeInfo = function (board, callback) {
...
db_utils.prototype.regNode = function (board, board_label, latitude, longitude, altitude, net_enabled, sensorsList, callback) {
    ...
    logger.debug("[SYSTEM] --> Node ID check...");

    connection.query("SELECT code FROM board_codes WHERE code =  '" + board + "'", function (err, result) {

        if (result.length == 0) {

            logger.debug("[SYSTEM] ----> Node " + board_label + " ("+board+") is not registered." );
                                    ...
                                    if (sensorsList.length > 0) {
                                        ...
                                        for (var i = 0; i < sensorsList2.length; i++) {
                                            ...
                                            (function (i) {
                                                ...
                                                connection.query("INSERT INTO sensors_on_board (id_sensor, id_board) VALUES ('" + sensorsList2[i] + "', '" + board + "')", function (err, latest_result) {
                                                    ...
                                                    if (i == (sensorsList2.length - 1)) {
                                                        logger.info("[SYSTEM] --> Registration of node " + board + " successfully completed!");
                                                        response.result = "SUCCESS";
                                        ...
                                        logger.info("[SYSTEM] --> Registration of the node " + board + " successfully completed!");
                                        response.result = "SUCCESS";
                                        response.message = "Registration successfully completed!";
                                        disconn();
                                        callback(response);
            ...
            response.message = "The node " + board_label + " ("+board+") already exists!";
...
//Function to register a new board
db_utils.prototype.updateNode = function (board, board_label, latitude, longitude, altitude, net_enabled, sensorList, callback) {
...
//Function to register a new board
db_utils.prototype.unRegNode = function (board, callback) {
...
db_utils.prototype.checkNode = function (node_id, callback) {
    conn();
    connection.query("SELECT * FROM user_boards WHERE user_boards.board_code ='" + node_id + "'", function (err, result) {
        if (err != null) {
            logger.error("checkNode: " + err);
        }
        disconn();
        callback(result);
    });
...
db_utils.prototype.checkNodeConnected = function (node_id, callback) {
    conn();
    ...
    connection.query("SELECT * FROM boards_connected WHERE boards_connected.board_code ='" + node_id + "'", function (err, result) {

        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            logger.error("checkNodeConnected: " + response.message);
            disconn();
            callback(response);
...
db_utils.prototype.insertNode = function (node_id, session, state, callback) {
    conn();
    connection.query("INSERT INTO boards_connected (board_code, session_id, status) VALUES ('" + node_id + "','" + session + "','" + state + "')", function (err, result) {
        if (err != null) {
            logger.error("insertNode: "+err);
        }
        disconn();
        callback(result);
    });
...
db_utils.prototype.changeNodeState = function (node_id, session, state, callback) {
    conn();
    connection.query("UPDATE boards_connected SET board_code='" + node_id + "',session_id='" + session + "',status='" + state + "' WHERE board_code='" + node_id + "'", function (err, result) {
        if (err != null) {
            logger.error("changeNodeState: " + err);
        }
        disconn();
        callback(result);
    });
...
db_utils.prototype.findBySessionId = function (session_id, callback) {
    ...
    connection.query("SELECT * FROM boards_connected WHERE boards_connected.session_id = '" + session_id + "'", function (err, result) {
        if (err != null) {
            logger.error("findBySessionId: " + err);
        }
        disconn();
        callback(result);
    });
...
db_utils.prototype.NodePlugins = function (node, callback) {
    conn();
    var response = {
        message: '',
        result: ''
    };
    connection.query("SELECT plugins.name, plugins.id, plugins.category, plugins_injected.state FROM plugins_injected, plugins WHERE plugins_injected.board_id = '" + node + "' AND plugins_injected.plugin_id = plugins.id", function (err_plugin, result_plugin) {
...
db_utils.prototype.removeAllServices = function (b_id, callback) {
    ...
    connection.query("DELETE FROM reverse_cloud_services WHERE reverse_cloud_services.board_id ='" + b_id + "'", function (err, result) {
        if (err != null) {
            logger.error("removeAllServices: " + err);
            response.message = err;
            response.result = "ERROR";
            disconn();
            callback(response);
...
db_utils.prototype.insertService = function (b_id, s_name, s_ip, s_port, callback) {
    ...
    connection.query("INSERT INTO reverse_cloud_services (board_id, service, public_ip, public_port) VALUES ('" + b_id + "','" + s_name + "','" + s_ip + "','" + s_port + "')", function (err, result) {
        ...
        if (err != null) {
            logger.error("insertService: " + err);
            response.message = err;
            response.result = "ERROR";
            disconn();
            callback(response);
...
db_utils.prototype.removeService = function (b_id, s_name, callback) {
    ...
    connection.query("DELETE FROM reverse_cloud_services WHERE reverse_cloud_services.board_id ='" + b_id + "' AND reverse_cloud_services.service='" + s_name + "'", function (err, result) {
        ...
        if (err != null) {
            logger.error("removeService: " + err);
            response.message = err;
            response.result = "ERROR";
            disconn();
            callback(response);
...
//Function to get the services started in a node
db_utils.prototype.NodeServices = function (node, callback) {
    conn();
    var response = {
        message: '',
        result: ''
    };
    connection.query("SELECT * FROM reverse_cloud_services WHERE reverse_cloud_services.board_id ='" + node + "';", function (err, result) {
        if (err != null) {
            logger.error("NodeServices: " + err);
            response.message = err;
            response.result = "ERROR";
            disconn();
            callback(response);
...
db_utils.prototype.insertVNET = function (vlan_ip, vlan_mask, net_uuid, vlan_name, callback) {
    ...
    logger.debug("[NETWORK] --> registering new VNET in DB...");
    ...
    connection.query("INSERT INTO vlans (vlan_ip, vlan_mask, net_uuid, vlan_name) VALUES ('" + vlan_ip + "', '" + vlan_mask + "', '" + net_uuid + "', '" + vlan_name + "' )", function (err, result) {
        ...
        if (err != null) {
            ...
            response.message = err;
            response.result = "ERROR";
            logger.error("[NETWORK] ----> insertVNET - INSERT error in vlans tables: " + response.message);
            disconn();
            callback(response);
            ...
            connection.query("SELECT id FROM vlans WHERE net_uuid='" + net_uuid + "'", function (err, result) {
                ...
                if (err != null) {
                    ...
                    response.message = err;
                    response.result = "ERROR";
                    logger.error("[NETWORK] ----> insertVNET - SELECT error in vlans tables: " + response.message);
                    disconn();
                    callback(response);
                    ...
                    response.message = result;
                    response.result = "SUCCESS";
                    logger.debug("[NETWORK] ----> insertVNET - new vlan ID ("+result[0].id+") selected from DB.");
                    disconn();
                    callback(response);
                }
...
db_utils.prototype.insertAddresses = function (list_ip, net_uuid, callback) {
    ...
    logger.debug("[NETWORK] --> Creating the addresses poll for the new VLAN " + net_uuid);
    ...
    connection.query(getVID, function (err, result) {
        ...
        if (err != null) {
            response.message = err;
            response.result = "ERROR";
            logger.error("[NETWORK] ----> insertAddresses - Error getting VNET id in vlans table: " + response.message);
            disconn();
            callback(response);
                    ...
                    if (i < list_ip.length - 1) 
                        ...
                        connection.query(query, function (err, result) {
                            if (err != null) {
                                ...
                                response.message = err;
                                response.result = "ERROR";
                                logger.error("[NETWORK] ----> insertAddresses - INSERT error in free_addresses table: " + response.message);
                                disconn();
                                callback(response);
                                ...
                                response.message = result;
                                response.result = "SUCCESS";
                                logger.debug("[NETWORK] ----> insertAddresses - New addresses pool successfully created for VNET "+net_uuid);
                                disconn();
                                callback(response);
...
//Function to get nodes info in a VNET
db_utils.prototype.getVnetNodesInfo = function (net_uuid, callback) {
...
db_utils.prototype.checkAddressPool = function (net_uuid, callback) {
    ...
    connection.query(query, function (err, result) {
        ...
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            logger.error("[NETWORK] --> checkAddressPool " + response.result + ": " + err);
            disconn();
            callback(response);
        }
        else {
            ...
            if (result.length != 0) {
                response.result = "SUCCESS";
                logger.debug("[NETWORK] --> " + JSON.stringify(result));
                disconn();
                callback(response);
            }
            else {
...
//Function to check if a node is already connected to a VNET
db_utils.prototype.checkNodeIntoVLAN = function (board, net_uuid, callback) {
    ...
    connection.query("SELECT EXISTS(  SELECT id_vlan, id_socat_connection FROM vlans_connection WHERE id_vlan = (SELECT id FROM vlans WHERE net_uuid='" + net_uuid + "' )  AND id_socat_connection=(SELECT id FROM socat_connections WHERE id_board='" + board + "' )  ) AS found", function (err, result) {
        ...
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            logger.error("Error checking node in VNET: " + err);
            disconn();
            callback(response);
        }
        else {
...
//Function to obtain the socat ID of a node
...
db_utils.prototype.getFreeAddress = function (net_uuid, callback) {
    ...
    connection.query(query, function (err, ip_result) {
        ...
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            logger.error("[NETWORK] --> " + response.result + " - " + err);
            disconn();
            callback(response);
...
db_utils.prototype.insertVnetConnection = function (vlan_ip, net_uuid, id_board, callback) {
    ...
    connection.query("SELECT EXISTS(SELECT 1 FROM vlans_connection WHERE ip_vlan ='" + vlan_ip + "' LIMIT 1) AS found", function (err, result) {
        if (err != null) {
            logger.warn("[NETWORK] ----> insertVnetConnection - Select error in checking ip_vlans table: " + err);
            response.message = err;
            response.result = 'ERROR';
            disconn();
            callback(response);
            ...
            if (result[0].found == 1) {
                logger.debug("[NETWORK] ----> insertVnetConnection - VNET connection already exists: " + result[0].id);
                response.message = result;
                ...
                //Socat ID related to the new node assigned "forever"
                connection.query("INSERT INTO vlans_connection (id_vlan, id_socat_connection,ip_vlan) VALUES ((select id from vlans where net_uuid='" + net_uuid + "'), (select id from socat_connections where id_board='" + id_board + "'), '" + vlan_ip + "')", function (err, result) {
                    ...
                    if (err != null) {
                        ...
                        logger.warn("[NETWORK] ----> insertVnetConnection - INSERT error in vlans_connection tables: " + err);
                        response.message = err;
                        response.result = 'ERROR';
                        disconn();
                        callback(response);
                        ...
                        logger.debug("[NETWORK] ----> insertVnetConnection - registering new VNET connection completed!");
                        response.message = result;
                        response.result = 'SUCCESS';
                        disconn();
                        callback(response);
    ...
    connection.query("SELECT id FROM socat_connections WHERE id_board='" + board + "'", function (err, result) {
        ...
        if (err != null) {
            ...
            if (result[0] == undefined) {
                ...
                logger.debug("[NETWORK] --> registering new node in DB...");
                ...
                connection.beginTransaction(function(err) {
                    ...
                    if (err) {
                        ...
                        response.result = "ERROR";
                        response.message = err;
                        logger.error("[NETWORK] --> getSocatConf transaction error: " + err);
                        callback(response);
                        ...
                        connection.query("INSERT INTO socat_connections (id_board, status, port, ip_board, ip_server) VALUES ('" + board + "', 'noactive', 0, 'None', 'None' )", function (err, result) {
                            ...
                            if (err != null) {
                                ...
                                connection.rollback(function() {
                                    response.result = "ERROR";
                                    response.message = err;
                                    logger.error("[NETWORK] --> Error inserting new Socat connection in socat_connections table: " + err);
                                    callback(response);
                                ...
                                logger.debug("[NETWORK] --> selecting new Socat node ID from DB: bSocatNum = " + bSocatNum);
                                ...
                                logger.debug("[NETWORK] --> creating new NET parameters in DB for node " + board + "..." + " port = '" + socatPort + "', ip_board='" + socatBoardAddr + "', ip_server='" + socatServAddr);
                                ...
                                connection.query("UPDATE socat_connections SET port = '" + socatPort + "', ip_board='" + socatBoardAddr + "', ip_server='" + socatServAddr + "'  WHERE id_board='" + board + "'", function (err, result_insert) {
                                    ...
                                    if (err != null) {
                                        ...
                                        connection.rollback(function() {
                                            response.result = "ERROR";
                                            response.message = err;
                                            logger.error("[NETWORK] --> Error updating new connection in socat_connections table: " + err);
                                            callback(response);
                                        ...
                                        connection.commit(function(err) {
                                            ...
                                            if (err) {
                                                ...
                                                connection.rollback(function() {
                                                    response.result = "ERROR";
                                                    response.message = err;
                                                    logger.error("[NETWORK] --> getSocatConf commit error: " + err);
                                                    callback(response);
                                                });
                ...
                */
                ...
                connection.query("SELECT id, ip_board, ip_server, port FROM socat_connections WHERE id_board='" + board + "'", function (err, result) {
                    ...
                    if (err != null) {
                        ...
                        //logger.debug("[NETWORK] --> Selecting Socat node ID from DB: bSocatNum = " + bSocatNum);
...
//Function to update the Socat status of a node
                    ...
                    if (err != null) {
                        ...
                        connection.query("INSERT INTO free_addresses (vlans_id, ip, insert_date) VALUES ( (SELECT id FROM vlans WHERE net_uuid='" + net_uuid + "' ), '" + board_ip + "', NOW())", function (err, result) {
                            ...
                            if (err != null) {
                                ...
                                logger.error("[NETWORK] --> removeBoardFromVlan - ERROR INSERT into free_addresses table: " + err);
                                response.message = err;
                                response.result = 'ERROR';
                                disconn();
                                callback(response);
                                ...
                                logger.debug("[NETWORK] --> removeBoardFromVlan - INSERT into free_addresses table SUCCESS!");
                                response.message = result;
                                response.result = 'SUCCESS';
                                disconn();
                                callback(response);
...
db_utils.prototype.getNodeVLAN = function (board, callback) {
    conn();
    var response = {
        message: '',
        result: ''
...
db_utils.prototype.insertPort = function (port, node_uuid, callback) {

    conn();
    logger.info("[NETWORK] --> Saving the port into the db " + port.id);

    var query = util.format("INSERT INTO ports (uuid,mac_address,ip_address,node_uuid,net_uuid) VALUES ('%s','%s','%s','%s','%s')", port.id, port.macAddress, port.fixedIps[0].ip_address, node_uuid, port.networkId);

    logger.debug("[NETWORK] --> query: " + query);
...
db_utils.prototype.getPort = function (net_uuid, node_uuid, callback) {
    conn();
    logger.info("[NETWORK] --> Getting the port for the node " + node_uuid + " on the network " + net_uuid);

    var query = util.format("select uuid from ports where node_uuid='%s' and net_uuid='%s'", node_uuid, net_uuid);
...
db_utils.prototype.deletePort = function (port, callback) {
    ...
    conn();
    logger.info("[NETWORK] --> Deleting the port into the db " + port);
...
//Function to get all Ports for a node
db_utils.prototype.getAllPorts = function (node_uuid, callback) {
    conn();
    logger.info("[NETWORK] --> Getting all the ports for the node " + node_uuid);

    var query = util.format("select * from ports where node_uuid='%s'", node_uuid);
    logger.debug("[NETWORK] --> query: " + query);
...
db_utils.prototype.insertMountpoint = function (node, mountpoint, src_node, src_path, status, callback) {
    ...
    connection.query("INSERT INTO mountpoints (src_board, src_path, dst_board, dst_path, status) VALUES ('" + src_node + "','" + src_path + "','" + node + "','" + mountpoint + "','" + status + "')", function (err, result) {
        if (err) {
            response.message = err;
            response.result = "ERROR";
...
db_utils.prototype.deleteMountpoint = function (node, mountpoint, src_node, src_path, callback) {
    conn();
    ...
    connection.query("DELETE FROM mountpoints WHERE dst_board='" + node + "' AND src_board='" + src_node + "' AND dst_path='" + mountpoint + "' AND src_path='" + src_path + "' ;", function (err, result) {
...
db_utils.prototype.NodeMountpoints = function (node, callback) {
    conn();
    var response = {
        message: '',
        result: ''
    };
    connection.query("SELECT dst_path, src_board, src_path FROM mountpoints WHERE dst_board = '" + node + "';", function (err, result) {
...
db_utils.prototype.checkNodeMountpoint = function (node, mountpoint, callback) {
    conn();
    var response = {
        message: '',
        result: ''
    ...
    connection.query("SELECT dst_path, src_board, src_path FROM mountpoints WHERE dst_board = '" + node + "' AND dst_path = '" + mountpoint + "' AND status = 'mounted';" , function (err, result) {
