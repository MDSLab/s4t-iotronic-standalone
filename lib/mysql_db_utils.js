...
db_utils.prototype.getSocatConf = function (board, basePort, socatNetwork, callback) {
    ...
    connection.query("SELECT id FROM socat_connections WHERE id_board='" + board + "'", function (err, result) {
        ...
        if (err != null) {
            ...
            logger.error("SELECT id error in socat_connections tables: " + err);
            disconn();
            callback(result);
            ...
            if (result[0] == undefined) {   //var socat_id = result[0].id;
                ...
                connection.query("INSERT INTO socat_connections (id_board, status, port, ip_board, ip_server) VALUES ('" + board + "', 'noactive', 0, 'None', 'None' )", function (err, result) {
                    ...
                    if (err != null) {
                        ...
                        logger.error("INSERT error in socat_connections tables: " + err);
                        disconn();
                        //callback(result);
                        ...
                        connection.query("SELECT id FROM socat_connections WHERE id_board='" + board + "'", function (err, result) {
                            ...
                            if (err != null) {
                                ...
                                logger.debug("[NETWORK] --> Selecting new board id from DB: bSocatNum = " + bSocatNum);
                ...
                connection.query("SELECT id, ip_board, ip_server, port FROM socat_connections WHERE id_board='" + board + "'", function (err, result) {
                    ...
                    if (err != null) {
                        ...
                        var bSocatNum = result[0].id - 1;
                        logger.info("[NETWORK] --> selecting new board id DB...bSocatNum = " + bSocatNum);
...
db_utils.prototype.insertNetVlan = function (vlan_ip, vlan_mask, net_uuid, vlan_name, callback) {
    ...
    logger.debug("[NETWORK] --> registering new VLAN in DB...");
    ...
    connection.query("INSERT INTO vlans (vlan_ip, vlan_mask, net_uuid, vlan_name) VALUES ('" + vlan_ip + "', '" + vlan_mask + "', '" + net_uuid + "', '" + vlan_name + "' )", function (err, result) {
        ...
        if (err != null) {
            ...
            logger.warn("[NETWORK] ----> insertNetVlan - INSERT error in vlans tables: " + err);
            disconn();
            ...
            connection.query("SELECT id FROM vlans WHERE net_uuid='" + net_uuid + "'", function (err, result) {
                ...
                if (err != null) {
                    ...
                    logger.warn("[NETWORK] ----> insertNetVlan - SELECT error in vlans tables: " + err);
                    disconn();

                } else {

                    logger.debug("[NETWORK] ----> new vlan ID selected from DB: " + result[0].id);
...
db_utils.prototype.insertVLANConnection = function (vlan_ip, net_uuid, id_board, callback) {
    ...
    connection.query("SELECT EXISTS(SELECT 1 FROM vlans_connection WHERE ip_vlan ='" + vlan_ip + "' LIMIT 1) AS found", function (err, result) {
        if (err != null) {
            logger.warn("[NETWORK] ----> insertVLANConnection - SELECT error in checking ip_vlans: " + err);
            disconn();
            response.message = err;
            response.result = 'ERROR';
            callback(response);
            ...
            if (result[0].found == 1) {
                logger.debug("[NETWORK] ----> insertVLANConnection - VLAN connection already exists: " + result[0].id);
                disconn();
                response.message = result;
                ...
                logger.debug("[NETWORK] ----> insertVLANConnection - registering new VLAN connection...");
                ...
                connection.query("INSERT INTO vlans_connection (id_vlan, id_socat_connection,ip_vlan) VALUES ((select id from vlans where net_uuid='" + net_uuid + "'), (select id from socat_connections where id_board='" + id_board + "'), '" + vlan_ip + "')", function (err, result) {
                    ...
                    if (err != null) {
                        ...
                        logger.warn("[NETWORK] ----> insertVLANConnection - INSERT error in vlans_connection tables: " + err);
                        disconn();
                        response.message = err;
                        response.result = 'ERROR';
                        callback(response);
                        ...
                        logger.debug("[NETWORK] ----> insertVLANConnection - registering new VLAN connection completed!");
                        disconn();
                        response.message = result;
                        response.result = 'SUCCESS';
                        callback(response);
...
db_utils.prototype.insertAddresses = function (list_ip, net_uuid, callback) {
    ...
    connection.query(getVID, function (err, result) {
        ...
        if (err != null) {
            ...
            logger.warn("[NETWORK] ----> insertAddresses - SELECT error in vlans table: " + err);
            ...
            connection.query(query, function (err, result) {
                if (err != null) {
                    ...
                    logger.warn("[NETWORK] ----> insertAddresses - INSERT error in free_addresses table: " + err);

                } else {
                    logger.debug("[NETWORK] --> New addresses pool successfully created!");
                    disconn();
                    callback(result);
                }
            });
...
db_utils.prototype.checkAddressPool = function (net_uuid, callback) {
    ...
    connection.query(query, function (err, result) {
        ...
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            logger.warn("[NETWORK] --> " + response.result + " - " + err);
            disconn();
            callback(response);
        }
        else {
            ...
            if (result.length != 0) {
                ...
                //logger.info(response.result + " - " + result);
                response.result = "SUCCESS";
                logger.debug("[NETWORK] --> " + response.result + " - " + JSON.stringify(result) + " err " + err);
                disconn();
                callback(response);
                    ...
                    if (err != null) {
                        ...
                        connection.query("INSERT INTO free_addresses (vlans_id, ip, insert_date) VALUES ( (SELECT id FROM vlans WHERE net_uuid='" + net_uuid + "' ), '" + board_ip + "', NOW())", function (err, result) {
                            ...
                            if (err != null) {
                                ...
                                logger.warn("[NETWORK] --> removeBoardFromVlan - ERROR INSERT into free_addresses tables: " + err);
                                disconn();
                                response.message = err;
                                response.result = 'ERROR';
                                callback(response);
                                ...
                                logger.debug("[NETWORK] --> removeBoardFromVlan - INSERT into free_addresses tables SUCCESS!");
                                disconn();
                                response.message = result;
                                response.result = 'SUCCESS';
                                callback(response);
