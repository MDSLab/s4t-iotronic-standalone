...
ckan_utils.prototype.getMap = function(req, res){
	...
	db.getBoardsConnected(function(data){  
	  ...
	  promiseWhile(function () { return board < data.length; }, function () {
			   ...
			   setTimeout(function(){
				...
				if (result.resources[resource].name != "metadata"){
				    ...
				    if (result.resources[resource].name == "temperature"){
					  ...
					  if(result[0]!= null){
					    logger.info("--> VALUE HUMIDITY of board "+id+": " + JSON.stringify(result[0].Humidity) +" percent");
					    response.boards[id]['resources'].metrics.push(result[0]);
					  }
					  ...
					  if(result[0]!= null){
					    logger.info("--> VALUE C02 of board "+id+": " + JSON.stringify(result[0].Gas) +" ppm");
					    response.boards[id]['resources'].metrics.push(result[0]);
					  }
					  ...
					  if(result[0]!= null){
					    logger.info("--> VALUE PRESSURE of board "+id+": " + JSON.stringify(result[0].Pressure) +" hPa");
					    response.boards[id]['resources'].metrics.push(result[0]);
					  }
	    ...
	    return Q.delay(300*data.length); // arbitrary async
