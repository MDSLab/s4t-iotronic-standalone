...
board_utils = function (session, rest) {
     ...
     *                  state:
     *                      type: string
     *                      description: "Board registration state: 'new' or 'registered'."
     *     responses:
     *       200:
    ...
    rest.post('/v1/boards/', function (req, res) {
        ...
        var ApiRequired= { "board_id":board_id, "board_label":board_label, "latitude":latitude, "longitude":longitude, "altitude":altitude,
            "net_enabled":net_enabled, "layout_id":layout_id, "user_id":user_id, "project_id":project_id, "mobile":mobile, "position_refr_time":position_refr_time, "notify":notify, "notify_rate":notify_rate, "notify_retry":notify_retry};
            ...
            if(check.result == "ERROR"){
                ...
                if(net_enabled != "true" && net_enabled != "false"){
                    ...
                    try {
                        ...
                        db.regBoard(board_id, board_label, latitude, longitude, altitude, net_enabled, sensorslist, layout_id, description, extra, project_id, user_id, mobile, position_refr_time, notify, notify_rate, notify_retry, function (db_result) {
                ...
                checkRequired(ApiRequired, function (check){
                    ...
                    if(check.result == "ERROR"){
                        ...
                        if (state != "new" && state != "registered"){
                            ...
                            db.updateBoard(board, board_label, latitude, longitude, altitude, net_enabled, sensorslist, layout_id, description, extra, project_id, user_id, mobile, position_refr_time,
                                notify, notify_rate, notify_retry, state, function (db_result) {
                        ...
                        if(data.result == "SUCCESS"){
                            ...
                            for (var i = 0; i < boards_list.length; i++) {
                                ...
                                (function (i) {
                                    ...
                                    execActionOnBoard(board_id, action, parameters, false);

                                    if (i === (boards_list.length - 1)) {

                                        response.result = "SUCCESS";
                                        response.message = "Action is executing on boards of the project '" + project + "'!";
                        ...
                        (function (i) {
                            ...
                            checkBoardAvailable(board_id, res, function (available) {
                                ...
                                if (available.result == "SUCCESS") {
                                    ...
                                    board_utils.injectConf(board_id, false);
                            ...
                            if (i === (boards_list.length - 1)) {
                                ...
                                response.result = "SUCCESS";
                                response.message = "Injecting conf on boards of the project '" + project + "'!";
                                logger.info("[PROJECT] --> " + response.message);
                                res.status(200).send(response);
            ...
            session_wamp.call('s4t.' + board + '.board.execAction', [action, parameters]).then(
                ...
                function (rpc_response) {
                    ...
                    if (rpc_response.result == "ERROR") {
                        ...
                        if(res != false){
                            res.status(500).send(response);
                            logger.error("[SYSTEM] --> Action error on board '" + board + "': " + response.message);
                        }
                        else{
                            logger.debug("[SYSTEM] --> Action error on board '" + board + "': " + response.message);
                        }
                        ...
                        if(res != false){
                            ...
                            logger.debug("[SYSTEM] --> Action result on board '" + board + "': " + response.message);
                        }
...
var createBoardConf = function (board_id, callback) {
    ...
    db.BoardInfo(board_id, function (data) {
        ...
        if (data.result == "ERROR") {
            ...
            settings.config.device = data.message.info.layout;
            ...
            //TEMPORARY--------------------------------------------------
            IoTronic_IP ="212.189.207.200";
            settings.config.wamp.port_wamp = "48181";
            settings.config.reverse.server.port_reverse = "48080";
            //-----------------------------------------------------------
