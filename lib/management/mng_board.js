...
board_utils = function (session, rest) {
                ...
                checkRequired(ApiRequired, function (check){
                    ...
                    if(check.result == "ERROR"){
                        ...
                        if (state != "new" && state != "registered"){
                            ...
                            db.updateBoard(board, board_label, latitude, longitude, altitude, net_enabled, sensorslist, type, description, extra, project_id, user_id, mobile, position_refr_time,
    ...
    //Execute action on project boards
    /**
     * @swagger
     ...
     */
    /*
    rest.post('/v1/projects/:project/action', function (req, res) {

        var board = req.params.board;

        logger.info("[API] - Project boards operation - " + Object.keys( req.route.methods ) + " - " + req.route.path + " - " + req.IotronicUser);
...
var createBoardConf = function (board_id, callback) {
    ...
    db.BoardInfo(board_id, function (data) {
        ...
        if (data.result == "ERROR") {
            ...
            var settings = JSON.parse(fs.readFileSync(__dirname+"/../../utils/board_settings_template_mod.json", 'utf8'));
