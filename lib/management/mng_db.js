...
db_utils.prototype.BoardInfo = function (board, callback) {
    ...
    connection.query("SELECT " +
        ...
        "WHERE boards.board_id = '" + board + "' AND coordinates.board_id = boards.board_id AND boards.layout_id = layouts.id_layout AND boards.projects_id = projects.uuid AND boards.users_id = users.uuid", function (err_info, result_info) {
        ...
        if (err_info != null) {
            ...
            connection.query("SELECT altitude, longitude, latitude, timestamp FROM coordinates WHERE coordinates.board_id = '" + board + "' ORDER BY timestamp DESC LIMIT 1", function (err_coord, coord) {
...
db_utils.prototype.regBoard = function (board, board_label, latitude, longitude, altitude, net_enabled, sensorsList, layout_id, description, extra, project_id, user_id, mobile, position_refr_time, notify, notify_rate, notify_retry, callback) {
    ...
    if (sensorsList != "empty" && sensorsList != "" && sensorsList != undefined)
    ...
    connection.query("SELECT board_id FROM boards WHERE board_id =  '" + board + "'", function (err, result) {
            ...
            connection.beginTransaction(function(err) {
                ...
                if (err) {
                    ...
                    connection.query("INSERT INTO boards (board_id, label, session_id, status, net_enabled, layout_id, description, extra, projects_id, users_id, mobile, position_refresh_time, " +
                        "notify, notify_rate, notify_retry, state) " +
                        "VALUES ('" + board + "', " +
                        "'" + board_label + "'," +
                        "'null'," +
                        "'D', " +
...
db_utils.prototype.updateBoard = function (board, board_label, latitude, longitude, altitude, net_enabled, sensorsList, layout_id, description, extra, project_id, user_id, mobile,
                                           position_refr_time, notify, notify_rate, notify_retry, state, callback) {
        ...
        if (result.length != 0) {
            ...
            connection.beginTransaction(function(err) {
                ...
                if (err) {
                    ...
                    connection.query("UPDATE boards, coordinates SET " +
                        "boards.label='" + board_label + "', " +
                        "boards.layout_id='" + layout_id + "', " +
                        "boards.description='" + description + "', " +
                        "boards.extra='" + extra + "', " +
                        "boards.state='" + state + "', " +
                        "boards.projects_id='" + project_id + "', " +
                        "coordinates.altitude='" + altitude + "', " +
                        "coordinates.longitude='" + longitude + "', " +
                        "coordinates.latitude='" + latitude + "', " +
                        "boards.users_id='" + user_id + "', " +
                        "boards.mobile='" + mobile + "', " +
                        "boards.net_enabled='" + net_enabled + "', " +
                        "boards.position_refresh_time='" + position_refr_time + "', " +
                        "notify='"+notify+"', " +
                        "notify_rate='"+notify_rate+"', " +
                        "notify_retry='"+notify_retry+"' " +
                        "WHERE boards.board_id='" + board + "' AND coordinates.board_id='" + board + "'", function (err, latest_result) {
...
db_utils.prototype.unRegBoard = function (board, callback) {
    ...
    connection.query("SELECT board_id FROM boards WHERE board_id =  '" + board + "'", function (err, result) {
        ...
        if (result.length != 0) {
            ...
            connection.query("DELETE FROM boards WHERE board_id='" + board + "';", function (err, result3) {
...
db_utils.prototype.unRegProjectBoards = function (project_id, callback) {
    ...
    connection.query("DELETE FROM boards WHERE projects_id='" + project_id + "';", function (err, result3) {
...
db_utils.prototype.checkBoard = function (board_id, callback) {
    ...
    connection.query("SELECT * FROM boards WHERE boards.board_id ='" + board_id + "'", function (err, result) {
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            disconn(connection);
...
db_utils.prototype.checkBoardConnected = function (board_id, callback) {
    ...
    connection.query("SELECT status FROM boards WHERE boards.board_id ='" + board_id + "'", function (err, result) {
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            disconn(connection);
...
db_utils.prototype.changeBoardWampStatus = function (board_id, session, status, callback) {
    ...
    connection.query("UPDATE boards SET board_id='" + board_id + "', session_id='" + session + "', status='" + status + "' WHERE board_id='" + board_id + "'", function (err, result) {
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            disconn(connection);
...
db_utils.prototype.changeBoardStatus = function (board_id, session, status, state, callback) {
    ...
    connection.query("UPDATE boards SET board_id='" + board_id + "', session_id='" + session + "', status='" + status + "', state='" + state + "' WHERE board_id='" + board_id + "'", function (err, result) {
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            disconn(connection);
...
db_utils.prototype.createLayout = function (model, manufacturer, image, layout, callback) {
    var connection = conn();
    var response = {
        message: '',
        result: ''
    };
    connection.query("INSERT INTO layouts (model, manufacturer, image, layout) VALUES (" + mysql.escape(model) + " , " + mysql.escape(manufacturer) + " , " + mysql.escape(image) + " , " + mysql.escape(layout) + ")", function (err, result) {
        if (err != null) {
            response.message = err;
            response.result = "ERROR";
            disconn(connection);
...
db_utils.prototype.updateLayoutById = function  (layout_id, model, manufacturer, image, layout, callback) {
    var connection = conn();
    var response = {
        message: '',
        result: ''
    };
    connection.query("UPDATE layouts SET model = '" + model + "', manufacturer = '" + manufacturer + "', image = '" + image + "', layout = '"+layout+"' WHERE id_layout=" + layout_id, function (err, result) {
        if (err != null) {
            response.result = "ERROR";
...
db_utils.prototype.getLayoutByBoardId = function (board_id, callback) {
    ...
    connection.query("SELECT layouts.id_layout, layouts.model, layouts.manufacturer, layouts.image FROM layouts, boards WHERE boards.layout_id = layouts.id_layout AND boards.board_id = '" + board_id + "'", function (err, result) {
...
db_utils.prototype.getProject = function (project, callback) {
    ...
    connection.query("SELECT * FROM projects WHERE uuid = '" + project + "' OR name = '"+project+"'", function (err, result) {
...
db_utils.prototype.getProjectBoards = function (project, callback) {
    ...
    connection.query("SELECT boards.* FROM projects, boards WHERE (projects.uuid = '" + project + "' OR projects.name = '"+project+"') AND projects.uuid = boards.projects_id", function (err, result) {
...
db_utils.prototype.getProjectUsers = function (project, callback) {
    ...
    connection.query("SELECT users.* FROM projects, users, boards WHERE (projects.uuid = '" + project + "' OR projects.name = '"+project+"') AND projects.uuid = boards.projects_id AND boards.users_id = users.uuid GROUP BY users.username", function (err, result) {
...
db_utils.prototype.getRequest = function (request_id, callback) {
    ...
    connection.query("SELECT * FROM requests WHERE id_request='"+request_id+"'", function (err, result) {
...
db_utils.prototype.getRequestResults = function (request_id, callback) {
    ...
    connection.query("SELECT board_id, result, message, timestamp FROM results WHERE request_id='"+request_id+"'", function (err, result) {
...
db_utils.prototype.getProjectRequestsList = function (project_id, callback) {
    ...
    connection.query("SELECT id_request, subject, result, timestamp FROM requests WHERE project_id='"+project_id+"'", function (err, result) {
...
db_utils.prototype.updateRequestStatus = function  (request_id, result, callback) {
    ...
    connection.query("UPDATE requests SET result = '" + result + "' WHERE id_request ='" + request_id +"'", function (err, result) {
        if (err != null) {
            response.result = "ERROR";
...
db_utils.prototype.updateResultStatus = function  (request_id, board_id, result, message, callback) {
    ...
    connection.query("UPDATE results SET result = '" + result + "', message = "+mysql.escape(message)+" WHERE request_id ='" + request_id +"' AND board_id ='" + board_id +"'", function (err, result) {
        if (err != null) {
            response.result = "ERROR";
...
db_utils.prototype.deleteRequest = function (request_id, callback) {
    ...
    connection.query("DELETE FROM requests WHERE id_request='" + request_id +"'", function (err, result) {
        if (err != null) {
            response.message = err;
            response.result = "ERROR";
            disconn(connection);
...
db_utils.prototype.getUser = function (user, callback) {
    ...
    connection.query("SELECT * FROM users WHERE uuid = '" + user + "' OR username = '"+user+"'", function (err, result) {
...
db_utils.prototype.getUserByBoard = function (board_id, callback) {
    ...
    connection.query("SELECT * FROM users, boards WHERE boards.users_id = users.uuid AND boards.board_id = '"+board_id+"'", function (err, result) {
...
db_utils.prototype.getUserBoards = function (user, callback) {
    ...
    connection.query("SELECT boards.* FROM users, boards WHERE (users.uuid = '" + user + "' OR users.username = '"+user+"') AND users.uuid = boards.users_id", function (err, result) {
...
db_utils.prototype.updateService = function  (service, serviceName, port, protocol, callback) {
    ...
    if (isNaN(service))
        var query = "UPDATE services SET name = '" + serviceName + "', port = '" + port + "', protocol = '" + protocol + "' WHERE name='" + service + "'";
    else
        var query = "UPDATE services SET name = '" + serviceName + "', port = '" + port + "', protocol = '" + protocol + "' WHERE id='" + service + "'";
...
db_utils.prototype.registerTunnel = function (service_name, board_id, public_port, pid, callback) {
    ...
    connection.query("SELECT id FROM services WHERE name = '" + service_name + "'", function (err, result) {
        ...
        if (err) {
            ...
            connection.query("INSERT INTO active_services (service_id, board_id, public_port, pid) VALUES ('" + service_id + "','" + board_id + "','" + public_port + "','" + pid + "')", function (err, result) {
                if (err) {
                    logger.error("registerTunnel: " + err);
                    response.message = err;
                    response.result = "ERROR";
...
db_utils.prototype.updateTunnelPid = function (service_id, board_id, pid, callback) {
    ...
    connection.query("UPDATE active_services SET pid = '" + pid + "', last_update=NOW() WHERE  service_id ='" + service_id + "' AND board_id ='" + board_id + "'", function (err, result) {
        if (err) {
...
db_utils.prototype.getPluginsChecksums = function (board_id, callback) {
    ...
    connection.query("SELECT plugins.name, plugins.checksum FROM plugins, plugins_injected WHERE plugins.id = plugins_injected.plugin_id AND plugins_injected.board_id = '" + board_id + "' ",
    ...
    if (isNaN(plugin))
        ...
        */
        var query = "SELECT plugins.id, plugins.name, plugins.category, plugins.version, plugins.checksum, plugins.code, plugins.defaults, plugins.checksum, plugin_tags.tag, plugin_tags.id as tag_id, plugin_types.type, plugins.type_id, " +
            "plugins.created_at, plugins.updated_at, plugins.description " +
            "FROM plugins, plugin_tags, plugin_types " +
            "WHERE plugin_types.id = plugins.type_id AND plugins.tag_id = plugin_tags.id AND plugins.id=" + plugin;
...
db_utils.prototype.checkPluginExists = function (name, version, category, type, callback) {
    ...
    var query = "SELECT * FROM plugins WHERE category=" + mysql.escape(category) + " AND type_id=" + type + " AND version=" + mysql.escape(version) + " AND name=" + mysql.escape(name);
...
db_utils.prototype.getUsedPlugin = function (plugin_id, callback) {
    ...
    connection.query("SELECT * FROM plugins_injected WHERE plugin_id = '" + plugin_id + "'", function (err, result) {
...
db_utils.prototype.getPluginVersions = function (plugin_name, callback) {
    ...
    connection.query("SELECT id, version FROM plugins WHERE name = '" + plugin_name + "'", function (err, result) {
        ...
        if (err) {
            ...
            connection.query("UPDATE plugins_injected SET state = '" + status + "', latest_change=NOW() WHERE board_id='" + board + "' AND plugin_id='" + plugin_id + "'", function (err, result) {
                if (err) {
                    ...
                    response.message = "Plugin '" + pluginname + "' status set to '" + status+"'";
                    response.result = "SUCCESS";
                    disconn(connection);
                    callback(response);
                }
...
db_utils.prototype.tagPlugin = function (plugin_id, tag_id, callback) {
    ...
    connection.query("UPDATE plugins SET tag_id = '" + tag_id + "', updated_at=NOW() WHERE id='" + plugin_id + "'", function (err, result) {
...
db_utils.prototype.BoardPlugins = function (board, callback) {
    ...
    connection.query("SELECT plugins.name, plugins.id, plugins.description, plugins.category, plugins_injected.state, plugin_tags.tag, plugin_types.type FROM plugins_injected, plugins, plugin_tags, plugin_types WHERE " +
        "plugins_injected.board_id = '" + board + "' AND plugins_injected.plugin_id = plugins.id AND plugins.tag_id = plugin_tags.id AND plugins.type_id = plugin_types.id ", function (err_plugin, result_plugin) {
...
db_utils.prototype.deleteInjectedPlugin = function (board, plugin_name, callback) {
    ...
    connection.query("DELETE FROM plugins_injected WHERE board_id='" + board + "' AND plugin_name='" + plugin_name + "'", function (err, result) {
...
db_utils.prototype.insertInjectedPlugin = function (board, plugin_id, plugin_name, callback) {
    ...
    connection.query("INSERT INTO plugins_injected (board_id, plugin_id, plugin_name, state) VALUES ('" + board + "','" + plugin_id + "','" + plugin_name + "', 'injected')", function (err, result) {
        if (err) {
...
db_utils.prototype.getTypeId = function (type, callback) {
    ...
    if (isNaN(type)) {
        ...
        connection.query("SELECT id, type FROM plugin_types WHERE type = '" + type + "'", function (err, result) {
        ...
        connection.query("SELECT id, type FROM plugin_types WHERE id=" + type, function (err, result) {
...
db_utils.prototype.getPluginFromVersion = function (plugin_name, plugin_version, callback) {
    ...
    connection.query("SELECT * FROM plugins WHERE version = '" + plugin_version + "' AND name = '" + plugin_name + "'", function (err, result) {
...
db_utils.prototype.getNetEnabledId = function (board, callback) {
    ...
    connection.query("SELECT net_enabled FROM boards WHERE board_id = '" + board + "'", function (err, result) {
