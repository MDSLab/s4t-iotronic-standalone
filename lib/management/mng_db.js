                ...
                if (err) {
                    ...
                    connection.query(up_query, function (err, latest_result) {
                        ...
                        if (err != null) {
                            ...
                            var sel_cty_query = "SELECT * FROM connectivity WHERE board_id=" + mysql.escape(board) + " AND main=1";

                            connection.query(sel_cty_query , function (err, sel_cty_result) {

                                if (err != null) {

                                    connection.rollback(function() {
                                        logger.error("[SYSTEM] --> (select) error in connectivity table: " + err);
                                        response.result = "ERROR";
                                        response.message = "(select) Error in connectivity table!";
                                        callback(response);
                                    });
                                    ...
                                    console.log(sel_cty_result);

                                    var main_cty = sel_cty_result[0].conn_id; // I got the main connectivity ID

                                    //if main_cty == cty.type:



                                    var up_cty_query = "UPDATE connectivity SET " +
                                        "conn_id=" + mysql.escape(cty.type) + ", " +
                                        "main=" + mysql.escape(cty.main) + ", " +
                                        "metadata=" + mysql.escape(JSON.stringify(cty.metadata)) + " " +
                                        "WHERE board_id=" + mysql.escape(board);

                                    connection.query(up_cty_query , function (err, cty_result) {
