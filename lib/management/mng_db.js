...
db_utils.prototype.getBoardsList = function (callback) {
    ...
    connection.query("SELECT * FROM boards b LEFT JOIN coordinates c ON c.board_id = b.board_code WHERE timestamp = (SELECT MAX(timestamp) FROM coordinates c2 WHERE c2.board_id = b.board_code)", function (err, result) {
        ...
        if (result.length == 0) {
            ...
            connection.beginTransaction(function(err) {
                ...
                if (err) {
                    ...
                    connection.query("INSERT INTO boards (board_code, label, session_id, status, net_enabled, type, description, extra, projects_id, users_id, mobile, position_refresh_time, notify, notify_rate, notify_retry) " +
                        "VALUES ('" + board + "', " +
                        "'" + board_label + "'," +
                        "'null'," +
                        "'D', " +
...
db_utils.prototype.unRegBoard = function (board, callback) {
    ...
    connection.query("SELECT board_code FROM boards WHERE board_code =  '" + board + "'", function (err, result) {
        ...
        if (result.length != 0) {
            ...
            connection.query("DELETE FROM boards WHERE board_code='" + board + "';", function (err, result3) {
