        ...
        if (result.length == 0) {
            ...
            connection.beginTransaction(function(err) {
                ...
                if (err) {
                    ...
                    connection.query("INSERT INTO boards (board_id, label, session_id, status, net_enabled, type, description, extra, projects_id, users_id, mobile, position_refresh_time, notify, notify_rate, notify_retry) " +
                        "VALUES ('" + board + "', " +
                        "'" + board_label + "'," +
                        "'null'," +
                        "'D', " +
...
db_utils.prototype.updateBoard = function (board, board_label, latitude, longitude, altitude, net_enabled, sensorsList, type, description, extra, project_id, user_id, mobile, position_refr_time, notify, notify_rate, notify_retry, callback) {
...
db_utils.prototype.changeBoardState = function (board_id, session, state, callback) {
    ...
    connection.query("UPDATE boards SET board_id='" + board_id + "',session_id='" + session + "',status='" + state + "' WHERE board_id='" + board_id + "'", function (err, result) {
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            disconn(connection);
...
db_utils.prototype.insertCreatedPlugin = function (name, category, parameters, code, version, type, description, callback) {
    ...
    connection.query("INSERT INTO plugins (name, category, version, checksum, type_id, tag_id, code, defaults, description, created_at) VALUES (" +
        ...
        mysql.escape(type) + " , " +
        mysql.escape("2") + " , " +   //tag "unreleased" by default at creation time
        mysql.escape(code) + " , " +
...
db_utils.prototype.getPlugin = function (plugin, callback) {
    ...
    if (isNaN(plugin))
        var query = "SELECT plugins.id, plugins.name, plugins.category, plugins.version, plugins.checksum, plugins.code, plugins.defaults, plugins.checksum, plugin_tags.tag, plugin_types.type, " +
            "plugins.created_at, plugins.updated_at, plugins.description " +
            "FROM plugins, plugin_tags, plugin_types " +
            "WHERE plugin_types.id = plugins.type_id AND plugins.tag_id = plugin_tags.id AND plugins.name=" + mysql.escape(plugin);
    else
        var query = "SELECT plugins.id, plugins.name, plugins.category, plugins.version, plugins.checksum, plugins.code, plugins.defaults, plugins.checksum, plugin_tags.tag, plugin_types.type, " +
            "plugins.created_at, plugins.updated_at, plugins.description " +
            "FROM plugins, plugin_tags, plugin_types " +
