...
db_utils.prototype.BoardInfo = function (board, callback) {
    ...
    connection.query("SELECT " +
        ...
        "WHERE boards.board_id = " + mysql.escape(board) + " AND coordinates.board_id = boards.board_id AND boards.layout_id = layouts.id_layout AND boards.projects_id = projects.uuid AND boards.users_id = users.uuid", function (err_info, result_info) {
        ...
        if (err_info != null) {
            ...
            connection.query("SELECT altitude, longitude, latitude, timestamp FROM coordinates WHERE coordinates.board_id = " + mysql.escape(board) + " ORDER BY timestamp DESC LIMIT 1", function (err_coord, coord) {
                ...
                if (err_coord != null) {
                    ...
                    connection.query("SELECT plugins.name, plugins.version, plugins.id, plugins.category, plugins_injected.state FROM plugins_injected, plugins WHERE plugins_injected.board_id = " + mysql.escape(board) + " AND plugins_injected.plugin_id = plugins.id", function (err_plugin, result_plugin) {
                        ...
                        if (err_plugin != null) {
                            ...
                            connection.query("SELECT sensors.type, sensors.model, sensors.id FROM sensors, sensors_on_board WHERE sensors_on_board.id_board = " + mysql.escape(board) + " AND sensors_on_board.id_sensor = sensors.id", function (err_sensor, result_sensor) {
...
db_utils.prototype.regBoard = function (board, board_label, latitude, longitude, altitude, net_enabled, sensorsList, layout_id, description, extra, project_id, user_id, mobile,
                ...
                if (err) {
                    ...
                    connection.query(up_query, function (err, latest_result) {
                        ...
                        if (err != null) {
                            ...
                            connection.query("DELETE FROM sensors_on_board WHERE id_board=" + mysql.escape(board) , function (err, result3) {
                                                    ...
                                                    connection.query(query, [values], function (err, latest_result) {
                                                        ...
                                                        if (err != null) {
                                                            ...
                                                            connection.rollback(function () {
                                                                ...
                                                                logger.error("[SYSTEM] --> " + response.message + ": " + err);
                                                                callback(response);
...
db_utils.prototype.changeBoardStatus = function (board_id, session, status, state, lr_version, callback) {
    ...
    connection.query("UPDATE boards SET board_id=" + mysql.escape(board_id) + ", session_id=" + mysql.escape(session) + ", status=" + mysql.escape(status) + ", state=" + mysql.escape(state) + ", lr_version=" + mysql.escape(lr_version) + " WHERE board_id=" + mysql.escape(board_id), function (err, result) {
        if (err != null) {
            response.result = "ERROR";
            response.message = err;
            disconn(connection);
