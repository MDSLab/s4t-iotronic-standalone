    ...
    if (wamp_conn_status == 'connection') {
        ...
        auth.checkAuthorization(board_id, function (auth_data) {
            ...
            if(auth_data.result == "SUCCESS"){
                ...
                //logger.warn(auth_data);
                iotronic_session.call('s4t.' + wamp_session + '.board.checkRegistrationStatus', [auth_data]);
        ...
        db.checkBoard(board_id, function (data) {
            ...
            if (data.result == "ERROR") {
                ...
                if (data.message.length == 0) {
                    ...
                    computeStatus(board_id, data.message[0].state, function (confStatus) {

                        if(confStatus.result == "SUCCESS"){

                            logger.info("[SYSTEM] --> Calling RPC board.checkRegistrationStatus...");

                            iotronic_session.call('s4t.' + board_id + '.board.checkRegistrationStatus', [confStatus]).then(
