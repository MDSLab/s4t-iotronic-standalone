...
try {
    ...
    check_board_rate = nconf.get('config:server:notifier:check_board_rate');
    enable_notify = nconf.get('config:server:notifier:enable_notify');
    retry_notify = nconf.get('config:server:notifier:retry');
                                        ...
                                        db.getUserByBoard(board, function (response) {
                                            ...
                                            if(response.result == "ERROR")
                                                ...
                                                if(board_notify == 0){
                                                    ...
                                                    logger.debug("[SYSTEM] - Alarm is DISABLED for the board " + board );

                                                }else{

                                                    logger.debug("[SYSTEM] - Alarm ACTIVATED for the board " + board);
                                                    ...
                                                    boards_disconnected[board] = setInterval(function(){
                                                        ...
                                                        if (alert_count <= retry_notify){

                                                            logger.debug("[SYSTEM] --> Alarm check ["+alert_count+"]:  Board " + board + " - User: "+user + " - Email: "+user_email); //+" - Cache: " + JSON.stringify(boards_disconnected));
                                                        ...
                                                        if (alert_count == retry_notify){
                                                            ...
                                                            logger.debug("[SYSTEM] - Alarm DEACTIVATED for the board: " + board);
