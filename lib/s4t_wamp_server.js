  ...
  connection.onopen = function (session, details) {
	...
	res.header('Access-Control-Allow-Origin','*');
        ...
        db.getBoardsConnected(function(data){
          response.list = data;
          res.send(JSON.stringify(response)); 
	  logger.debug("Board list called.");
        });
	...
	logger.info("[MAIN] - Server REST started on: http://"+IPLocal+":"+restPort+ " status " + iotronic_status);
	logger.info("[MAIN] - Connected to router WAMP!");
    }
    else{
	logger.error("[MAIN] - ERROR: "+ iotronic_status);
    }
		...
		logger.warn("[MAIN] - A not authorized board has tried a connection to the cloud!");
		    ...
		    db.checkBoardConnected(args[0], function(data){
		      ...
		      if(data.length == 0){
				...
				logger.debug("Now the status of the board is:");
				logger.debug("--> board_code::"+db_board_id);
				logger.debug("--> session::"+data[0].session_id + " - status::"+data[0].status);  
