...
try {
    ...
    admin_email = nconf.get('config:server:admin_email:address');
    admin_email_pw = nconf.get('config:server:admin_email:password');
    check_board_rate = nconf.get('config:server:check_board_rate');
                                    ...
                                    if (data.result == "ERROR") {
                                        ...
                                        db.getBoard(board_id, function (data) {
                                            ...
                                            if (data.result == "ERROR") {
                                                ...
                                                logger.info("[SYSTEM] - Board " + data.message[0].label + " (" + board_id + ") CONNECTED!");
                                                ...
                                                if ( boards_disconnected[board_id] != undefined){
                                                     ...
                                                     logger.debug("[SYSTEM] - Alarm DEACTIVATED for board: " + board_id);
                                                }
                ...
                db.findBySessionId(session_id, function (data_find) {
                    ...
                    if (data_find.result == "ERROR") {
                        ...
                        if (data_find.message.length == 0) {
                            logger.warn("[WAMP] --> A not registered board disconnected!");
                        }
                        else{
                            ...
                            db.changeBoardState(board, 'null', 'D', function (data) {
                                ...
                                if (data.result == "ERROR") {
                                    ...
                                    logger.debug("[SYSTEM] - Alarm ACTIVATED for board " + board);

                                    db.getUserByBoard(board, function (response) {

                                        if(response.result == "ERROR")
                                            logger.error("[SYSTEM] - Error getting user email: " +response.message);
                                            ...
                                            logger.debug("[SYSTEM] - Owner user of board " + label + " (" + board + "): " + user + " ("+user_email+")");
                                            ...
                                            boards_disconnected[board] = setInterval(function(){
                                                ...
                                                logger.debug("[SYSTEM] --> Alarm check ["+alert_count+"]:  Board " + board + " - User: "+user_email+" - Cache: " + JSON.stringify(boards_disconnected));

                                                /*
                                                if (alert_count == 1){

                                                    utility.sendEmail(smtpConfig, user_email);

                                                    clearInterval( boards_disconnected[board] );
                                                    delete boards_disconnected[board];
                                                    logger.debug("[SYSTEM] - Alarm DEACTIVATED for board: " + board);

                                                }
                                                */

                                            }, check_board_rate * 1000);
