...
s4t_wamp_server.prototype.start = function(restPort, wamp_router_url, wamp_realm){
  ...
  connection.onopen = function (session, details) {
	...
	brgre_up.stderr.on('data', function (data) {
	    logger.warn('--> gre bridge up stderr: ' + data);
	});
		...
		db.checkBoardConnected(board, function(data){
		  ...
		  if(data.length == 1){
			...
			error:{}
			  ...
			  case 'driver':
			      //http://212.189.207.205:8888/command/?command=driver&drivername='.$driver_name.'&driverjson='.$driver_json.'&driveroperation=mount&board='.$board_id)
			      //plugin_utils.manageDrivers(board, drivername, driverjson, driveroperation, res);
			      ...
			      driver_utils.manageDrivers(board, drivername, driveroperation, res);
			      break			      
    ...
    //VLAN RECONNECTION ON JOIN BOARD
