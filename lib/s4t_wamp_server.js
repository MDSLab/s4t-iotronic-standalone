  ...
  connection.onopen = function (session, details) {
        ...
        db.checkBoardConnected(board, function(data){
          if(data.length == 1){
            if(data[0].status == 'off'){
              //DEBUG
              console.log("Board state is off");
              var response = {
                error:{}
              }
              response.error="Board state is off"
              ...
              case 'measure':
                ...
                console.log(board+'.command.rpc.'+mname+':::' +pin+"," +op+"," +measuren+"," +measuret);
                session.call(board+'.command.rpc.'+mname, [mname, op, mtime]).then(
                  function(result){
                  response.result = result;
                  res.send(JSON.stringify(response));
                } , session.log);
    ...
    var onLeave_function = function(session_id){
      ...
      db.findBySessionId(session_id, function(data){
        ...
        if(data.length == 1){
          db.changeBoardState(data[0].board_code,'null','off', function(result){
            //DEBUG
