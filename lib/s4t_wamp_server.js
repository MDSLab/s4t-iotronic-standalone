			...
			(function(resource) {
				...
				if (result.resources[resource].name != "metadata"){
				  ...
				  if (result.resources[resource].name == "temperature"){
				    ...
				    getCKANwithParam("http://smartme-data.unime.it/api/3/action/datastore_search",{"resource_id":result.resources[resource].id, "limit":1, "sort":"Date  desc"}).then(
				      function(result){
					if(result[0]!= null){
...
*/
    ...
    rest.get('/test/', function (req, res){
	  ...
	  for (var board = 0; board < data.length; board++) {
		...
		promises.push( getCKAN(id).then(
		  
		      function(result) {
			// Do stuff
			console.log("Getting metrics of board "+ id);

			response.boards[id]['resources'] = {metrics:[]};
			getCKANresources(result, response, id).then(function() {
			    console.log("RESOURCE STOP of " + id);
			});
	  ...
	  Q.all(promises).then(function() {
	      // Do all the things!
	    console.log("FINITOOOOOOOOOOOOOOOOOOO: "+ JSON.stringify(response));
	    res.send(JSON.stringify(response));
	  })
    ...
    function getCKANresources(result, response, id){    
				...
				if (result.resources[resource].name != "metadata"){
				  ...
				  if (result.resources[resource].name == "temperature"){
				    ...
				    getCKANwithParam("http://smartme-data.unime.it/api/3/action/datastore_search",{"resource_id":result.resources[resource].id, "limit":1, "sort":"Date  desc"}).then(
				      function(result){
					if(result[0]!= null){
					  console.log("--> VALUE TEMP of board "+id+": " + JSON.stringify(result[0].Temperature) + " °C");
					  response.boards[id]['resources'].metrics.push(result[0]);
				    ...
				    getCKANwithParam("http://smartme-data.unime.it/api/3/action/datastore_search",{"resource_id":result.resources[resource].id, "limit":1, "sort":"Date  desc"}).then(
				      function(result){
					if(result[0]!= null){
					  console.log("--> VALUE LUX of board "+id+": " + JSON.stringify(result[0].Brightness) +" lux");
					  response.boards[id]['resources'].metrics.push(result[0]);
					  d.resolve();
					}
...
/*      
	  ...
	  promises.push(getCKAN("14141414").then(function(result) {
		  // Do stuff
		//response.boards[board] = {"id":board}; 
		console.log(board);
	      })
	  );
      }
      ...
      .then(function() {
	  // Do all the things!
	console.log("FINITOOOOOOOOOOOOOOOOOOO!");
	//console.log("FINITOOOOOOOOOOOOOOOOOOO: "+ JSON.stringify(response));
	//res.send(JSON.stringify(response));
      })
...
*/    
    ...
    function getCKANwithParam(restUrl, restParams){    
	...
	console.log("Server REST started on: http://"+IPLocal+":"+restPort+ " status " + iotronic_status);
	console.log("Connected to router WAMP");
    }
    else{
	console.log("ERROR: "+ iotronic_status);
    }
    ...
    console.log("Subscribed to topic: "+topic_connection);

    var onLeave_function = function(session_id){

      console.log("SESSION closed with code: "+session_id);
