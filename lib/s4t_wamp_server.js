    ...
    connection.onopen = function (session, details) {
        ...
        if (IPLocal != undefined) {
            ...
            var onBoardConnected = function (args) {
                        ...
                        if (data.length == 0) {
                            ...
                            db.checkNodeConnected(board_id, function (data) {
                                ...
                                if (data.length == 0) {
                                    ...
                                    logger.info("[SYSTEM] - First Connection of the Board " + board_id);
                                    db.insertNode(board_id, wamp_session, 'C', function (result) {
                                        logger.debug("[SYSTEM] - Board DB insert result: " + result);
                                    });
                ...
                //else logger.error("[WAMP] - Wrong board ("+board_id+") connection status:\n"+args)
            ...
            var Provisioning = function (result) {
                ...
                db.getNodePosition(node_id, function (node_position) {
                    logger.info('[SYSTEM] - BOARD POSITION: ' + JSON.stringify(node_position) + ' PROVISIONING NODE RESPONSE: ' + result[0]);
