        ...
        if (IPLocal != undefined) {
            ...
            var onBoardConnected = function (args) {
                ...
                if (wamp_conn_status == 'connection') {
                    ...
                    logger.debug("[WAMP] - Board ("+board_id+") connection status: "+args)
            ...
            var result_network_board = function (result) {
                ...
                /*
                 ...
                 (function (i) {
                 logger.debug("[VNET] - " + JSON.stringify(result_db[i], null, "\t"));
            ...
            session.call("wamp.session.list").then(
                function (list) {
                    ...
                    //logger.info(list)
                    //logger.info(list.indexOf(parseInt(session.id)))
                    delete list[list.indexOf(parseInt(session.id))];
                    logger.info("[WAMP] - Board connected sessions: " + list);
                    ...
                    if (subCommandId != null){
                        ...
                        session.call("wamp.subscription.list_subscribers",[subID]).then(
                            ...
                            function (subBoards) {
                                ...
                                logger.info("[WAMP] - Active sessions on WAMP: " + subBoards);
                                                    ...
                                                    if(wrong_conn_boards.length !=0){
                                                        ...
                                                        for (var w = 0; w < wrong_conn_boards.length; w++) {
                                                            ...
                                                            (function (w) {
                                                                ...
                                                                logger.warn(" --> "+wrong_conn_boards[w]);
                                                                //Input parameters: board_id - res = false - restore = false
                                                                    ...
                                                                    if (data.result == "ERROR") {
                                                                        ...
                                                                        db.removeAllServices(wrong_board, function (response) {
                                                                            ...
                                                                            if(response.result == "ERROR")
                                                                                logger.error("[WAMP] -->  Error removing all active services (board "+wrong_board+"): " +response.message);
                                                                            else {
                                                                                //logger.debug(" --> Removed all active services!");
                                                                                ...
                                                                                logger.info("[WAMP] --> " + wrong_board + ": DB status updated!");
                                ...
                                if (data.result == "ERROR") {
                                    ...
                                    db.updateSocatStatus(board, "noactive", function (response) {
                                        ...
                                        if(response.result == "ERROR")
                                            ...
                                            logger.debug("[VNET] - Socat status of board " + board + " set to 'noactive' ");
