    ...
    connection.onopen = function (session, details) {
        ...
        if (API_IP != undefined) {
            ...
            if ( (docs_embedded == "true"  || docs_embedded == true)  && (docs_exp == "false"  || docs_exp == false) ){
                logger.info("[API-DOCS] - API documentation system is enabled in embedded mode.");
                docs.genApiDocs(rest);
            }else if ( (docs_embedded == "false"  || docs_embedded == false)  && (docs_exp == "true"  || docs_exp == true) ) {
                logger.info("[API-DOCS] - API documentation system is enabled in exposed mode.");
                docs.exposeApiDocs(rest, null);
            }else if ( (docs_embedded == "true"  || docs_embedded == true)  && (docs_exp == "true"  || docs_exp == true) ) {
                logger.warn("[API-DOCS] - API documentation system is disabled: both embedding and exposing system are set as enabled, choose only one of them!")
            }else if ( (docs_embedded == "false"  || docs_embedded == false)  && (docs_exp == "false"  || docs_exp == false) ) {
                logger.info("[API-DOCS] - API documentation system is disabled!")
            }
            ...
            rest.get('/', function (req, res) {
                ...
                if ( (docs_embedded == "true"  || docs_embedded == true)  && (docs_exp == "false"  || docs_exp == false) ){
                    ...
                    res.status(200).send("<center> Welcome in Iotronic-standalone! <br><br><br> <a href="+docs_url+">Please visit API documentation</a> </center>");
            ...
            logger.error("[SYSTEM] - SERVER IP UNDEFINED: please specify a valid network interface or a public IP in settings.json");
            logger.info('Bye!');
            process.exit();
