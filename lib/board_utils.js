...
board_utils = function (session, rest) {
        ...
        checkRestInputs(req, function (check){
            ...
            if(check.result == "ERROR"){
                ...
                var APIparamsList= {"board_label":board_label, "board_id":board_id, "latitude":latitude, "longitude":longitude, "altitude":altitude,
                    "net_enabled":net_enabled, "type":type, "user_id":user_id, "project_id":project_id, "mobile":mobile, "position_refr_time":position_refr_time, "notify":notify};
                            ...
                            try {
                                ...
                                if (req.body.extra != undefined) {
                                    ...
                                    if(isJSON){
                                        ...
                                        db.regBoard(board_id, board_label, latitude, longitude, altitude, net_enabled, sensorslist, type, description, project_id, user_id, mobile, position_refr_time, notify, function (db_result) {
                                            ...
                                            if (db_result.result === "SUCCESS") {
                                                ...
                                                if (ckan_enabled === "true") {
                                                    ...
                                                    ckan.CkanBoardRegistration(board_id, board_label, latitude, longitude, altitude, function (ckan_result) {
    ...
    rest.patch('/v1/boards/:board', function (req, res) {
        ...
        checkBoardAvailable(board, res, function (available) {
            ...
            if (available.result != "ERROR") {
                ...
                db.updateBoard(board, board_label, latitude, longitude, altitude, net_enabled, sensorslist, type, description, project_id, user_id, mobile, position_refr_time, notify, function (db_result) {
    ...
    rest.post('/v1/boards/:board/position', function (req, res) {
        ...
        logger.debug("[API] - Add board position - " + Object.keys( req.route.methods ) + " - " + req.route.path);
    ...
    rest.get('/v1/boards/:board/position', function (req, res) {
        ...
        if (window == undefined){
            ...
            if (samples > 1){
                logger.debug("[API] - Get board position history - " + Object.keys( req.route.methods ) + " - " + req.route.path);
            }
    ...
    */
    ...
    rest.get('/v1/boards/:board/position', function (req, res) {
        ...
        logger.debug("[API] - Get board position - " + Object.keys( req.route.methods ) + " - " + req.route.path);

        if (window != undefined && samples != undefined) {
            response.message = "You have to specify or 'samples' or 'window' parameter";
            response.result = "ERROR";
            logger.error("[API] --> " + response.message);
            res.send(JSON.stringify(response));

        }else if (samples != undefined && window == undefined){
            ...
            logger.warn("OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO")

            db.getBoardPositionHistory(board_id, function (data) {
