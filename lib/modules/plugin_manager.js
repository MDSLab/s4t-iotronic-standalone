                ...
                db.getUsedPlugin(p_id, function (data) {
                    ...
                    if (data.result == "ERROR") {
                        ...
                        if (data.message[0] === undefined) {
                            ...
                            logger.debug("[PLUGIN] --> Plugin " + plugin + " is not injected in any board");
                            ...
                            response.message = "Plugin " + plugin + " can not be deleted: it is still injected in some boards!";
                            response.result = "ERROR";
                            logger.warn("[PLUGIN] --> " + response.message);
                            res.status(500).send(response);
                                            ...
                                            session_wamp.call('s4t.'+ board + '.plugin.run', [plugin_name, pluginjson, plugin_checksum]).then(
                                                ...
                                                function (rpc_response) {
                                                    ...
                                                    if (rpc_response.result == "ERROR") {
                                                        ...
                                                        response.result = "ERROR";
                                                        res.status(500).send(response);
                                                        logger.error("[PLUGIN] --> Running error for '" + plugin_name + "' plugin: " + JSON.stringify(response.message));
