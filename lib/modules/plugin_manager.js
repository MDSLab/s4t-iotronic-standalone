...
plugin_utils = function (session, rest) {
     ...
     *                          type_id:
     *                              type: string
     *                              description: "Plugin type ID: identifies the nature of the plugin, 'nodejs' or 'python'"
     *                          tag_id:
     *                              type: string
     *                              description: "Plugin revision ID"
     *       403:
     ...
     *                  checksum:
     *                      type: string
     *                      description: "Plugin checksum"
     *                  type_id:
     *                      type: number
     *                      description: "Plugin type ID (e.g. '1' -> 'nodejs | '2' -> 'python')"
     *                  tag_id:
     *                      type: number
     ...
     *                  description:
     *                      type: string
     *                      description: "Plugin description (300 chars)"
     *     produces:
     *       - application/json
     *     responses:
     *       200:
                        ...
                        db.getInjectedPlugin(pluginId, board, function (data) {
                            ...
                            if (data.result == "ERROR") {
                                ...
                                if (data.message.length == 0) {
                                    ...
                                    logger.debug("[PLUGIN] --> Calling injectplugin RPC with name = '" + pluginName + "' onboot = '" + autostart + "' code: \n" + pluginCode +
                                        "\nwith the following default parameters:\n"+pluginParameters);

                                    //Now I can perform the RPC call
                                    session_wamp.call('s4t.'+ board + '.plugin.inject', [JSON.stringify(pluginBundle) , autostart]).then(
