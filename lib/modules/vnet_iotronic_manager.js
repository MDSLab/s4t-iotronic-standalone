...
net_utils = function (session, rest) {
     ...
     *     tags:
     *       - Virtual Networks
     *     description: It returns IoTronic VNETs list
    ...
    rest.put('/v1/boards/:board/vnets/:vnet', function (req, res) {
        ...
        board_utility.checkBoardAvailable(board, res, function (available){
            ...
            if(available.result == "SUCCESS"){
                ...
                net_utils.addToNetwork(vnet, board, value, res, restore);
     ...
     *     tags:
     *       - Virtual Networks
     *     description: Force VNET funcionality enabling on board
     *     summary: force VNET enabling on board
     *     parameters:
     *      - in: path
     *        name: board
     *        required: true
...
function destroyVLAN(net_uuid, res, response) {
    ...
    db.destroyVNET(net_uuid, function (db_response) {
        ...
        if (db_response.result == "ERROR") {
            ...
            res.status(500)(JSON.stringify(db_response, null, "\t"));
                    ...
                    if (net_enabledID == 1) {
                        ...
                        db.checkAddressPool(netuuid, function (free_address) {
                            ...
                            if(free_address.result == "ERROR"){
                                ...
                                res.status(199).send(JSON.stringify(response, null, "\t"));
                                    ...
                                    if (network.result == "ERROR") {
                                        ...
                                        if (network.message.length != 0) {
                                            ...
                                            if (netsize >= 1) {
                                                ...
                                                logger.error("[VNET] --> " + response.message );
                                                res.status(500)(JSON.stringify(response, null, "\t"));
                                            }
...
function createEstablishTunnels(elem, res, response, restore) {
    ...
    session_wamp.call('s4t.'+ elem.board + '.vnet.addToNetwork', [elem.board, elem.vlanID, elem.greIP, elem.greMask, elem.vlan_name]).then(
