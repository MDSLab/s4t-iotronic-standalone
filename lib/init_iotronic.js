...
try {
    ...
    logger.debug('[SYSTEM] - EMERGENCY LOG');
...
var initIoTronicModules = function() {
    ...
    if(nconf.get('config:server:docs:exposed:url') == "env"){
        docs_url = process.env.SWAGGER_URL;
        logger.debug("[SYSTEM] - URL API docs exposed at (env):", docs_url);
    }
    else{
        docs_url = nconf.get('config:server:docs:exposed:url');
    }

    if(nconf.get('config:server:docs:expose:url_spec') == "env"){
        docs_url_spec = process.env.SWAGGER_SPEC_URL;
        logger.debug("[SYSTEM] - URL API docs specs exposed at (env):", docs_url_spec);
    }
    else{
        docs_url_spec = nconf.get('config:server:docs:expose:url_spec');
    }
