...
var initIoTronicModules = function() {
    ...
    wstun_ssl = nconf.get('config:wstun:ssl');
    wstun_keypath = nconf.get('config:wstun:key');
    wstun_crtpath = nconf.get('config:wstun:crt');
    wstun_port = nconf.get('config:wstun:port');
    ...
    */
    ...
    try{
        logger.info('[WSTUN] -> Creating wstun reverse server')
        const config = !wstun_ssl ?  {} : {
            ssl: true,
            key: wstun_keypath,
            crt: wstun_crtpath
        } 
        wstun_reverse_server = new wstun.server_reverse(config)
        wstun_reverse_server.start(wstun_port)
        logger.info(`[WSTUN] -> reverse server listening on port ${wstun_port}`)
    }
    catch(err){
        logger.error('[WSTUN] -> ', err)
    }
    */

    // Create fork for wstun instead of daemon
    function startWsTun(){
        const wstun = wstun_ssl ? spawn('wstun', ['-r', '-s', '8080', '--ssl=true', `--key="${wstun_keypath}"` `--cert=${wstun_crtpath}`]) : spawn('wstun', ['-r', '-s', '8080']);
        logger.info('[WSTUN] -> child process created')
        wstun.stdout.on('data', onData);
        wstun.on('close', onClose)
    }
    function onData(data){
        logger.info('[WSTUN] -> '+data)
    }
    function onClose(code){
        logger.info('[WSTUN] -> exited with code:'+code)
        logger.info('[WSTUN] -> restartting service')
        startWsTun()
    }
    startWsTun()
