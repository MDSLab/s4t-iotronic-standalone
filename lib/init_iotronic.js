...
var initIoTronicModules = function() {
    ...
    if(nconf.get('config:wstun:port_range:high') == "env" && nconf.get('config:wstun:port_range:low') == "env"){
        wstun_h_port = process.env.WSTUN_H_PORT;
        wstun_l_port = process.env.WSTUN_L_PORT;
        console.log("- WSTUN ports range:",wstun_h_port, wstun_l_port)
    }
    else {
        wstun_h_port = nconf.get('config:wstun:port_range:high');
        wstun_l_port = nconf.get('config:wstun:port_range:low');
        console.log("- WSTUN ports range:",wstun_h_port, wstun_l_port)
