  ...
  if(op=="start" || op=="stop"){
    ...
    if(op=="start"){
      db.checkService(board, command, function(data){
        if(data.length == 0){
          //DEBUG
          logger.info("Service "+command+" never started!");
          ...
          newPort(function(port){
            ...
            session_wamp.publish(topic_command, [board, command, port,op], {}, {acknowledge: true}).then(
                function(publication){
                    logger.info('publication went okay!');
                },
                function(error){
                    logger.error('publication error!');
                }
            );
            response.port = port;
            response.status=op;
    ...
    if(op=="stop"){
      ...
      db.checkService(board,command, function(data){
        if(data.length == 0){
          logger.info("It not already started!");
          res.send(JSON.stringify(response));
        }
        else{
          //db.getPortService(board,command, function(data){
            //DEBUG
            logger.info("Service "+command+ " stopped!");
            response.port = data[0].public_port;
