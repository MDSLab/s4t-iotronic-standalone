...
utility.prototype.exportService = function(board, command, op, res){
  ...
  if(op=="start" || op=="stop"){
    ...
    if(op=="start"){
      db.checkService(board, command, function(data){
        if(data.length == 0){
          ...
          //DEBUG
          logger.warn("[SERVICE] - Service already started on board "+board+"!");
    ...
    if(op=="stop"){
      ...
      db.checkService(board,command, function(data){
        if(data.length == 0){
          logger.warn("[SERVICE] - SSH service is already stopped on board "+board+"!");
          res.send(JSON.stringify(response));
        }
        else{
          //db.getPortService(board,command, function(data){
            //DEBUG
            logger.info("[SERVICE] - Service "+command+ " stopped on board "+board+"!");
            response.port = data[0].public_port;
